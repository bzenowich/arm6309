...
**The partition, fitted 2026-09-07** — `hardware/gal/cpld/`, regenerated by
`hardware/gal/prjbureau/fit1508.sh`. Both parts **fit on the fitter's first pass**,
without the cascade logic the audio card needed:

| | Holds | Macrocells | Signals placed |
|---|---|---|---|
| **`vaddr`** | scan and `WPTR` counters, and the address mux | 53 — **34 buried** | 58 of 68 |
| **`vctrl`** | sync trio, sequencer, span control, arbiter, `CTRL` | 62 — **30 buried** | 63 of 68 |

The fitter put `DOTCLK` on pin 83, a global clock, and `RESET` on pin 1, the global
clear, on both parts. 74,136 fuses each.

**The cut is where it is for a timing reason, not a capacity one.** A net crossing
between packages costs a `tPD` — 15 ns at the `-15` grade against a 39.7 ns dot period
— so every dot-rate signal is on `vctrl` with whatever it clocks: the phase counter,
`SLOTTICK`, `FCLK0..3`, `MUXSEL`. `vaddr` is the address datapath and 17 of its
outputs are the framebuffer address, which is what makes it the natural boundary.

> ⚠ **These two parts hold what is written, and v1 is more than that.** Neither
> carries §19 item 23's decode half (~13 macrocells and roughly 21 pins), §10.1.5's
> tile and character modes (+8 I/O), or §10.3's list engine (+35 macrocells). The
> macrocells are there — `vctrl` would land near 118 of 128 — but **the pins are not**:
> at 63 of 68 it has four spare and the decode half alone needs about twenty.
>
> **The room is on `vaddr`, and it is being wasted.** It spends **17 pins on `HS2..HS9`
> and `VS0..VS8`**, the scroll preload values, because that is how the GAL fits
> declared them — and `D0..D7` is *already* one of its inputs. Loading the scroll off
> the register-file data path with a strobe per slice, as §9.5 does for `WPTR` on the
> audio card, takes `vaddr` from 57 pins to about 40. That is the first move before
