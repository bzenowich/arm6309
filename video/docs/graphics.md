...
| 1 | `74HCT574` | `TASK` at `$FFB0` — one live bit of eight. **MMU enable and the shadow-ROM disable are both gone**: `hardware/gal/README.md` findings 1 and 3 showed the first cannot exist and the second could not be wired, and §7.2's `BOOT` at `$FFB1` replaced what the second was for |
...
does not walk until the blank ends, so its acknowledgement lands first. `software/demo/gui.asm`'s `golist` serves the request itself while
...
3. ⭐ **Post the write and retire it at the next horizontal blank — taken 2026-09-14, at
   ...
   line each; `boot.asm`'s `palw`, `software/demo/`'s palette loaders and NitrOS-9's
...
ROM — is `hardware/ram.md` §6.5, at **18 ICs**):
...
44. **⭐ CLOSED 2026-09-11 — the three posted-write address `'574`s are deleted, and `WPTR`
    ...
    **The card is 33 ICs** (§14.1). `hardware/place/parts.ts` and
    `cards/video.circuit.tsx` carry it, `npm run check:place` still places it on 24 cm
...
48. **⭐ CLOSED 2026-09-13 — cell mode's map-code handover follows the scrolled cell
    ...
    **Checked three ways, and each fails on the old equations.** `vtile_tb` walks a line
    ...
    `software/demo/` on the whole machine: every game frame equals the model.
