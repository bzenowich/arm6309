...
thing. The current fit is **`vctrl` 123 of 128 and `vaddr` 113 of 128** — the
...
stood; it is 52 of 64 I/O and 123 of 128 cells today (§14 has the live figure).
...
- **Crossing delay.** A net between packages costs a `tPD` — 15 ns at the `-15` grade,
  ...
  logic cells and 58 of 64 I/O**, and 35 of 40 LAB fan-in — so the choice is no longer free.
...
|---|---|---|---|---|
| **`vaddr`** | PLCC-84 | scan and `WPTR` counters (`WPTR` doubling as the list engine's pointer, §10.3.1), scroll and tile registers, **the map byte's two-stage pipeline**, the write-strobe decode, the four-source address mux, **§7.2's reload walk**, **§10.3.2's descriptor engine** | **113 of 128** | **58 of 64** |
...
> ⚠ **No CPLD on this card reserves JTAG** (§10.1.6.3). `vaddr` is at 58 of 64 I/O
...
strobe, the map pipeline and §7.2's reload walk. ⚠ `vctrl` is the tight one: 123 of
128 cells, with pins to spare. §14.2's two ×16 framebuffer parts are still the relief
...
33. **⚠ OPEN — §5.2's rewrite to §14.2's two ×16 framebuffer parts.** Raised as an
    ...
    **123 of 128 cells**. A by-four increment is a **mode-dependent carry** — from bit 0
    ...
    are not a problem… `vctrl` sits at 64 of 64" — `vctrl` is at **52 of 64 pins and
