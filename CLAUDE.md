...
| ⭐ **`npm run check:machine`** | **the whole machine**: a 6809E core, the motherboard and the video card, running `software/boot/boot.asm` out of the boot ROM, and a screenshot taken off `RGB`/`BLANK`/`HSYNC`/`VSYNC`. Four runs: the boot, and three `+scenario=` faults (no SIMM, an aliasing module, a corrupted VRAM byte) in which the ROM's error path is the asserted answer. **86 claims, ~2 min**; `SCENARIOS=main` for the boot alone |
