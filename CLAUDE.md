...
| ⛔ ~~`sh gal/verilog/run-demo.sh`~~ | **RETIRED 2026-09-22 to [`archive/video/bench/`](archive/video/bench/)**, with `demo_tb.sv`, `run-demo-emu.sh`, `run-replay.sh`, `run-calib.sh` and the two `run-vramrate` scripts. They ran `software/demo/`'s show on the **archived `video` card**, which nothing builds against any more. ⚠ `software/demo/` itself STAYS: `mktbox.py` generates the ROM toolbox's icon art from its drawing code, and `software/demo/emu/machine.c` is the host emulator every live bench runs |
...
| ⭐ **`sh video3/bench/run-scroll.sh`** | ⭐⭐ **A WORLD OF ANY SIZE, AND THE RING CHECKED BYTE FOR BYTE.** `scroll` (`video3/docs/scrolling.md`) is the other answer to the problem `zelda` solves by being a room game: the camera roams a 4096 × 2048 world in **`VMODE` 11's square 640 × 480**, and the tile bank lives in the **384 × 512 rectangle of ring the raster never looks at** — rotating through it one 32-column strip at a time so it is never in the way. ⭐ **The gate is the invariant, not a screenshot**: `checkscroll.py` walks the same path the program walks, works out where all nine strips ended up, and compares **all 524,288 bytes**. ⛔ **With two mutations that are REQUIRED to fail** — the strip not moved, and the vertical band not composed. **~12 min**; needs `../nitros9` on its `arm6309` branch, so it is in no aggregate |
...
| the **archived** `video` card (`video.cpld.ts`, `vsup.cpld.ts`, `sync`/`scan`/`access`/`seqph`/`seqctl`/`regfile`/`vlen`/`pxsel`) | ⛔ **NOTHING RUNS IT AT ALL SINCE 2026-09-22.** Its five benches and `demo_tb` are in `archive/video/bench/`, `check:machine` moved to `video3` with the boot ROM on 2026-09-20, and the NitrOS-9 recipe cannot build against it (`-DV3=1` is in `AFLAGS`). ⚠ The **design sources are still in `hardware/gal/`** and `gen.ts` still emits their Verilog — compiled, and executed by nothing. `archive/README.md` §"What did NOT move" is the record of that trade |
...
| ⭐ `pcs`, the scan converter, the ball or any part proc | ⭐ **`sh video3/bench/run-pcs.sh`** (~8 min), and ⛔ **run `python3 video3/bench/pcsobj.py` first** - the model carries its own self-tests and a broken transliteration is not worth six minutes on a 6809. ⚠ The model is never corrected to agree with the 6809; when they differ the 6502 decides |
| ⭐ `desk.asm`, the window move, or anything the desktop shell draws | ⭐ **`sh video3/bench/run-v3move.sh`** (~14 min) and `sh video3/bench/run-v3files.sh` (~11 min) — the second is what catches a manager whose hit testing and drawing have drifted apart, because `SetX`/`SetY` and `FileHit` are the same two words read in opposite directions |
...
| ⭐ the storage card, `boot.asm` §10b, `boot_sd.asm`, `mksddisk.sh` or where `OS9Boot` lives | `TBS=storage sh run.sh` (~20 s), then ⭐ **`sh software/nitros9/run-sdboot.sh`** (~4 min), and ⭐ **`SCENARIOS="disk nodisk" npm run check:machine`** — which is the only thing that cold-starts §10b, because the host emulator enters at `$8004` and reaches it only after a `reboot` |
...
```sh
TBS=v3card sh run.sh         # one; TBS="audio storage" for several
```
...
|---|---|---|
| ⛔ ~~`vsync_tb`, `vaddr_tb`, `vtile_tb`, `vspan_tb`, `vpal_tb`, `demo_tb`~~ | the **archived** `video` card | **RETIRED 2026-09-22 to `archive/video/bench/`**, where their README records what each proved. They are frozen and do not run from there |
