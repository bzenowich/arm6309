...
| ⭐ **`npm run check:reach`** | **every signal the machine produces must reach something.** `design-review2.md` closed the direction "a fitted part reads what nothing produces"; this is the other one — a signal that is *produced* and that nothing reads, which for a register bit means **a feature the host can write and the card cannot perform**. ⭐ **And since 2026-09-18 the same question of INPUTS**, for a card with no board file: video3's four parts turned out to be a datapath and an arbiter with **neither sequencer built** — 19 control lines nothing produces. Part of `npm run check` |
