...
| `sh gal/verilog/run-demo.sh 27` | **the whole machine with BOTH cards**, running `software/demo/` from ROM, then: the card's register stream against refplayer, the card's sound A/B'd against libopenmpt, every frame against the source picture and the game model, and an H.265 file. ⚠ **~2 h**, and not in any aggregate. `software/demo/bench/run-replay.sh` is the replayer alone on the CPU, in two minutes |
