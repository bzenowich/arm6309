...
docs/modplayer.md §9 argues for a corpus where each module fails loudly for one
...
def probes(outdir):
    ...
    def emit(name, rowfn, songlen=1):
        rows = []
        for r in range(64 * songlen):
            cs = blank()
            rowfn(r, cs)
            rows.append(cs)
        made.append((name, build(os.path.join(outdir, name + ".mod"), rows, songlen=songlen)))
