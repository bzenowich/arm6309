/* Model of the arm6309 sound card. See card.h and docs/audio.md. */
...
void card_reset(card_t *c, uint8_t *sram, uint32_t sram_bytes)
{
    ...
    /* docs/audio.md §13: reset leaves the card quiet — display of the audio
...
void card_load_linear_lut(card_t *c)
{
    ...
    for (unsigned v = 0; v < 128u; v++) {
        ...
        for (unsigned s = 0; s < 256u; s++) {
             ...
             * bottom two bits go. docs/audio.md §6.1 argues they are below the
...
void card_write(card_t *c, uint8_t reg, uint8_t val)
{
    ...
    case A_ADATA: {
        ...
        if (off <= ST_PAN) {
            c->state[i] = val;
            if (off == ST_DAT) {
                /* Direct sample write, docs/audio.md §1 requirement 8. */
            ...
            for (unsigned n = 0; n < 4u; n++) {
                if ((mask >> n) & 1u) {
                    if (!c->ch[n].dmaen) {
                         ...
                         * shadow. docs/audio.md §16 item 13 bounds the delay,
...
uint8_t card_read(card_t *c, uint8_t reg)
{
    ...
    case A_ADATA: {
         ...
         * so they never stall — docs/audio.md §9.3. */
        return c->state[i];
    }
    case A_AINTREQ:
        return c->intreq;
...
static void chan_tick(card_t *c, unsigned n)
{
    ...
    if (ch->ptr < c->sram_bytes && c->sram) {
         ...
         * (docs/modplayer.md §4.6), which is worth failing a test over. */
        byte = 0;
        c->oob_reads++;
    }
    ...
    if (ch->att) {
         ...
         * test corpus exercises it. See docs/audio.md §11.3. */
    ...
    if (--ch->cnt == 0u) {
        /* THE shadow reload — docs/audio.md §3.3, requirement 3. It takes
...
void card_step(card_t *c)
{
     ...
     * runs at the Amiga's CIA rate — docs/audio.md §8.2. */
...
void card_dac(const card_t *c, int *l, int *r)
{
    ...
    /* Hard-panned, 0 and 3 left, 1 and 2 right — docs/audio.md §1 requirement
