/* The ProTracker replayer — docs/modplayer.md §5.
 *
...
{
    /* docs/audio.md §8.2: the timer counts at colourclock/5 — the Amiga's CIA
...
/* docs/modplayer.md §5.3 — the one sequence that must be exactly right. */
static void commit_triggers(mod_player *p)
{
     ...
     *    one-shot runs out (docs/audio.md §3.3). ProTracker's delay loop is not
...
static void e_tick0(mod_player *p, unsigned n)
{
    ...
    case 0x0:
         ...
         * other than a channel register (docs/modplayer.md §5.6). x=0 turns the
    ...
    case 0x6:
        if (x == 0u) {
            ...
            if (ch->loop_cnt) {
                 ...
                 * processed wins (docs/modplayer.md §5.8). */
                p->loop_pending = 1;
    ...
    case 0xF:
         ...
         * Unimplemented by decision, not by omission — docs/modplayer.md §10.8
         * and §11 item 3. */
        break;
...
/* docs/modplayer.md §5.8. The order of these steps is where players diverge. */
...
void mod_start(mod_player *p, mod_song *s, card_t *c)
{
     ...
     * precisely so this order is possible (docs/audio.md §9.2). */
    for (unsigned n = 0; n < MOD_CHANNELS; n++) {
