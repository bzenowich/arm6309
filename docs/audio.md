...
([`plan.md`](plan.md)) and a 256-colour card with tilemaps and a span writer
([`graphics.md`](graphics.md)). What does the sound card look like, given that
...
> **This supersedes [`graphics.md`](graphics.md) §17's sound-card paragraph**, which
...
architectural decision in [`graphics.md`](graphics.md) — the 4-way interleave, the
...
vertical blank ([`graphics.md`](graphics.md) §5.2).
...
[`graphics.md`](graphics.md) §6.4.2 takes for the 8bpp tilemap — a deliberate,
...
| The video card's 512 KB VRAM | shares a scheduled resource | **Ruled out** — [`graphics.md`](graphics.md) §17 already rejects this for the DOC card, and every word of that reasoning transfers. |
...
[`graphics.md`](graphics.md) §3.1, and with far more slack. Upload cost is in §13.2.
...
[`graphics.md`](graphics.md) §17 puts both `/IRQ` and `/FIRQ` on the backplane
...
[`graphics.md`](graphics.md) §13 takes **`$FF60–$FF7F`** (32 bytes) for video, on
...
([`graphics.md`](graphics.md) §17), so the base is a jumper, not a wire.
...
This is exactly [`graphics.md`](graphics.md) §11's `VDATA` prefetch argument, and
...
[`graphics.md`](graphics.md) §17 assumed this part, and that assumption should be
...
**Because [`plan.md`](plan.md) §4.1 forbids it.** The bus loop is hard real time:
...
([`graphics.md`](graphics.md) §12.2 restates this for the raster-compare
...
[`graphics.md`](graphics.md) §16 gives for the bus exerciser: it decouples the
...
That is exactly the A/B lever [`plan.md`](plan.md) §6.2 builds the whole
...
~5 core cycles per store that [`graphics.md`](graphics.md) §7.3 uses — and with
...
([`graphics.md`](graphics.md) §7.3). Music plays underneath the console with
room to spare.
...
[`graphics.md`](graphics.md) §10.2's bulk RAM movement, and both of them exist
...
| 5 | **Discrete card rev A**, driven by the STM32 bus exerciser ([`graphics.md`](graphics.md) §16.1) — no 6309 core needed | state file reads back; a single channel plays a sine from card RAM at a known `PER` |
...
1. **Confirm the CPU store rate.** §13.1's ~3 % and §13.2's 187 ms both scale on
   ...
   [`graphics.md`](graphics.md) §19 item 1 already flags. Measure it once and both
...
7. **GAL fit, and the 8-channel question** (§11.2). The sequencer carries the
   ...
   mode. Same posture as [`graphics.md`](graphics.md) §19 item 15.
...
[`graphics.md`](graphics.md) §15 — and, if §12.5's bring-up card ends up shipping,
...
- [`graphics.md`](graphics.md) — §3.1 (posted writes), §5 (one clock, static
  ...
  document supersedes)**.
- [`plan.md`](plan.md) — §2.1 (edge-driven bus loop), §4.1 (no hardware ISR — the
...
- `~/code/colormin/docs/backplane.md` — slot model, open-drain interrupt lines,
  the mono `AUDIO` node that [`graphics.md`](graphics.md) §17 already argues should
  be stereo.
