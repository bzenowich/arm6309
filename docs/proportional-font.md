...
`video3/docs/demo-report.md` §16, `docs/boot-and-desktop.md` §5 item 8 and
`video3/docs/keyed-copy.md` §7.1; figures marked **estimated** are cycle counts
read off the source at E = 2.0979 MHz, and §9 says which measurement settles
them. `video3/docs/keyed-copy.md` §3.2 and `docs/boot-and-desktop.md` §5 item 8
...
runs 6309 code (`docs/6309.md` §4.1). It is worth **1.11×, not the 1.9× §5.4
...
From the built blob (`software/nitros9/video/build3/tbox.bin`, parsed
...
`docs/boot-and-desktop.md` §5 item 8 (2026-09-21) says:
...
⭐ **CHECKED, 2026-09-21, and it is now a gate**: `gal/video3/regmap.check.ts`
(`npm run check`) walks all 32 offsets and requires each to be *decoded by a live
...
`gal/video3/v3dot_si.pld` and `v3host_st.pld` *do* decode them — with real
...
`docs/6309.md` §4.1) runs `TFM`, `LDW`/`STW`, `LDQ`/`STQ`, `ADDW`/`SUBW`/`CMPW`,
...
| ⭐ **`TFM` to `VDATA`** | a different question and a real one — `optimizations.md` §9: 3 cycles a byte against `vidcore`'s 16. It is now runnable, and ⭐ **`TFM`'s interrupt behaviour is decided** (`cpu/docs/plan.md` §4.3.1, option C: the byte completes before the interrupt, so nothing is lost against a side-effecting port) |
| ⭐ **`-DH6309=1`**, unrelated to text | the kernel's own 6309 paths stop being compiled out. ⭐ **`CPU=6309` now implies it** (`docs/6309.md` §4.3); the two used to be separate knobs and setting one alone compiled every `ifne H6309` out silently |

⛔ **What still cannot run it is everything else.** `machine3.v` instantiates an
`mc6809e`, and `cpu/`'s own core has no decoder. So a 6309 measurement is a
