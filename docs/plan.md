...
> address deadline. That was wrong. The replacement used `docs/MC6809E.pdf`, whose
...
> is now in the repo: `docs/HD6309E_datasheet.pdf` p.3 gives AC characteristics for
...
**Datasheet timing — `docs/HD6309E_datasheet.pdf` p.3, "AC CHARACTERISTICS".**
...
Also worth recording, since `docs/MC6809E.pdf` disagrees: Motorola specifies `t_DHR` as
...
`tools/microstate-probe/` compiles candidate microstates with the firmware's exact flags
...
Component costs, tuned hot path (`tools/microstate-probe/tuned.S`):
...
4. **~~Overclock the G4 to 200 MHz.~~ EVALUATED AND REJECTED (2026-08-28).**
   ...
   **You cannot overclock the core without also overclocking the PLL VCO, and there is
   ...
   is 344 / 2 = 172 MHz.** `src/clock.c` already runs the VCO at 340 MHz — *98.8% of the
...
> including 1.79 MHz** (`tools/microstate-probe/`). Two indirect calls cost ~8 cycles of
...
|---|---|---|
| ~~Microcode step costs more than 15–30 cycles~~ | **Largely closed** | Measured: 14 cy typical, 25 cy worst, against 34 available at 3 MHz (§3.3(d), `tools/microstate-probe/`). Residual risk is coverage — see the two rows below. |
...
3. ~~What are `t_DSR` and `t_DHR`?~~ **Answered, twice.** First from `docs/MC6809E.pdf`
   p.3 (40 ns / 10 ns, MC68B09E), then properly from `docs/HD6309E_datasheet.pdf` p.3,
...
3d. ~~Does the microcode step fit the budget?~~ **Answered — yes, with the §4.1 rules.**
   ...
   (`tools/microstate-probe/`, §3.3(d)). The 15–30 estimate was pessimistic. But the
...
- **HD63B09E / HD63C09E datasheet** — `docs/HD6309E_datasheet.pdf`. **The authoritative
  bus-timing source for this project** (p.3, AC characteristics, both grades in adjacent
  columns). Supersedes `docs/MC6809E.pdf` for everything in §3.3.
- STM32G431CB datasheet DS12589 Rev 6 — `docs/stm32g431kb.pdf`. Table 45 (PLL
...
- **RM0440 Rev 9** — `docs/stm32g4-refman.pdf`. §9.3.1 (GPIO `IDR` free-running),
  Table 2 §1.6 (no FSMC/QUADSPI on G431), §13.4.4–13.4.5 (DMAMUX sync/request generator).
  Sources for §3.4(6).
- **SAM D5x/E5x family datasheet** — `docs/ATSAMD51G19A.pdf`. §52 (PCC), §32 (PORT behind
...
- MC6809E datasheet — `docs/MC6809E.pdf`. Retained for the Motorola part actually in the
