#!/bin/sh
# ⭐ THE FILE MANAGER, CLICKED AT - arm6309 software/desk/docs/boot-and-desktop.md §3,
# milestone 3.
#
#   sh software/desk/bench/run-v3files.sh       ~11 min.  OUT=dir, NOBUILD=1, NORUN=1
#
...
#      software/desk/docs/boot-and-desktop.md §5 item 6
...
cd "$(dirname "$0")/../../.."
ROOT=$(pwd)
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3files}
...
eval "$(python3 software/desk/bench/checkfiles.py --geom "$DESKASM")"
...
run() {
  ...
  python3 software/desk/bench/checkfiles.py "$DESKASM" "$D" "$OUT/names.txt" \
