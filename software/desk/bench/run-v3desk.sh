#!/bin/sh
# ⭐ THE DESKTOP, DRIVEN BY A MOUSE - arm6309 software/desk/docs/boot-and-desktop.md §3,
# milestones 1 and 2.
#
#   sh software/desk/bench/run-v3desk.sh        ~3 min.  OUT=dir, NOBUILD=1, NORUN=1
#
# ⛔ WHAT LOOKED LIKE A DESKTOP UNTIL NOW WAS A RECORDING.  `v3desk`,
# `v3menu` and `v3paint` are scripted streams of CoArm escapes that
# software/paint/tools/v3show.py writes and `copy /sd0/data/v3desk /w3`
...
# card and clicks at it with PS2_SCRIPT (software/emu/ps2script.h) -
...
cd "$(dirname "$0")/../../.."
ROOT=$(pwd)
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3desk}
S="$ROOT/software/desk/bench/scripts"
DESKASM=${DESKASM:-}
...
run() {   # run <dir> <ps2 script> <desk ticks> <seconds of machine>
  ...
  python3 software/desk/bench/checkdesk.py "$DESKASM" "$D" > "$D/desk.txt" 2> "$D/desk.err" \
...
claim "⭐ AND ON TO ITEM 2 (Stardew), which was the greyed one until 2026-09-21" \
  test "$(hiat click 168 72)" = 2
...
# itself has its own bench (arm6309 software/stardew/bench/run-v3star.sh).
claim "⭐ Stardew is on the menu, and the source declares it RUNNABLE" \
  grep -q '^item 1 2 1 Stardew stardew' "$OUT/click/desk.txt"
