...
# software/desk/docs/boot-and-desktop.md §3.5 item 4.
#
#   sh software/desk/bench/run-v3move.sh        ~7 min.  OUT=dir, NOBUILD=1, NORUN=1
...
#      the backing store's whole job (software/desk/docs/boot-and-desktop.md §3.7)
...
cd "$(dirname "$0")/../../.."
ROOT=$(pwd)
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3move}
RUNS=${RUNS:-"drag idle"}
...
if [ -z "$NOBUILD" ]; then
  ...
  OUT="$OUT" DATA="$OUT/data" sh software/nitros9/mksyscard.sh "$OUT/sd.img" desk v3paint monster stardew v3bbs v3art \
...
run() {   # run <dir> <CONTROL or "">
  ...
  SESSION=1 CONTROL="$2" python3 software/desk/bench/mkdragps2.py > "$D/drag.ps2"
  ...
  CONTROL="$2" python3 software/desk/bench/checkmove.py "$DESKASM" "$D" \
