...
I/O rather than by a bare-metal ROM.  hardware/video3/docs/demo-report.md 4 says why.
...
Files:
  ...
  v3art       Blocktronics' we-tortuga, re-emitted - only when software/paint/reference/we-tortuga.ans is there
...
def sktext(x, y, s, ramp="panel", bold=False):
    ...
    the strike.  software/toolbox/docs/proportional-font.md §4.
...
def textp(x, y, s, ramp="panel", bold=False, opaque=False):
    ...
    replaced; the ratios in software/toolbox/docs/proportional-font.md §6.1 are against this."""
    fnt = int(bold) | (F_OPAQ if opaque else 0)
...
ART = pathlib.Path(__file__).resolve().parents[1] / "reference" / "we-tortuga.ans"
...
def have_art():
    ...
    the same way software/archive/demo's photograph is not: put it in hardware/video3/ (it is in
...
def stream_art():
    ...
    # ⭐ WE-TORTUGA, AND NOTHING ELSE, SINCE 2026-09-24.  This project's own
    # piece drew here for two days and is archived
    # (software/archive/paint/ansi-original.py); main() leaves `v3art` out
...
def main(out):
    ...
    # ⭐ AND SINCE 2026-09-24 IT IS CONDITIONAL AGAIN: `v3art` is we-tortuga,
    # which is Blocktronics' and not in this repository, so a clone without
    # software/paint/reference/we-tortuga.ans has no `v3art` and its desktop
    # icon forks nothing.  The owner's choice; the home-made piece that filled
    # the gap is in software/archive/paint/.
    if have_art():
        streams[6:6] = [("v3pal", stream_pal), ("v3art", stream_art)]
    else:
        print("note  no %s: v3art is left out (software/paint/reference/README.md)" % ART.name)
