...
#
#   sh software/stardew/bench/run-v3star.sh        ~25 min.  OUT=dir, FRAMES=n, ACTORS=n
#
# `stardew` (software/stardew/docs/stardew.md, and the port's
...
cd "$(dirname "$0")/../../.."
ROOT=$(pwd)
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3star}
FRAMES=${FRAMES:-1050}
ACTORS=${ACTORS:-7}
...
if [ -z "$NOBUILD" ]; then
  python3 software/stardew/bench/mkstardew.py "$NITROS9DIR/level2/arm6309/cmds" \
    > "$OUT/mkart.log" 2>&1 || {
...
FD="$OUT/farmdata"
rm -rf "$FD"; mkdir -p "$FD"
cp software/stardew/bench/stardew.pic "$FD/"
...
python3 software/stardew/bench/checkv3star.py $DIRS || fail=1
...
if [ -z "$NOMUTATE" ]; then
  ...
  NOSHEET=1 python3 software/stardew/bench/checkv3star.py --gate-must-fail $MD || fail=1
fi
