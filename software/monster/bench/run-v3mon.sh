...
#
#   sh software/monster/bench/run-v3mon.sh          ~25 min.  OUT=dir, FRAMES=n, MODES=...
#
...
cd "$(dirname "$0")/../../.."
ROOT=$(pwd)
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3mon}
FRAMES=${FRAMES:-1050}
SPEED=${SPEED:-4}
ACTORS=${ACTORS:-12}
...
if [ -z "$NOBUILD" ]; then
  python3 software/monster/bench/mkmonster.py ../nitros9/level2/arm6309/cmds > "$OUT/mkart.log" 2>&1 || {
...
python3 software/monster/bench/checkv3mon.py $DIRS || fail=1
...
if [ -z "$NOMUTATE" ]; then
  ...
  NOSHEET=1 python3 software/monster/bench/checkv3mon.py --gate-must-fail $MD || fail=1
fi
