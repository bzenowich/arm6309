...
OUT=${OUT:-obj_sd}
mkdir -p "$OUT"
cc -O2 -w -I"$ROOT/hardware/audio/refplayer" -o "$OUT/sdtest" sdtest.c "$ROOT/hardware/cpu/sim/cpu6809.c" "$ROOT/hardware/cpu/sim/hd6309.c" "$ROOT/hardware/audio/refplayer/card.c"
"$OUT/sdtest" "$OUT"
