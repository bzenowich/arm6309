 ...
 *
 *   emu ROM.bin OUTDIR SECONDS        (software/emu/run-emu.sh builds it)
 *
 ...
 *     the register file);
 *   - the audio card: hardware/audio/refplayer/card.c, the register-level model with
 ...
 *   - the TL16C550C at $FF38 (hardware/io/serial/docs/serial.md 7): divisor latch,
 ...
 *     boots NitrOS-9 here;
 *   - the storage card at $FF58-$FF5B (hardware/storage/docs/sdcard.md 6.2) and an
 ...
 *     hardware/storage/sim/storage_card.v and sd_model.v, driven by
...
typedef struct {
    ...
    /* ⭐ video3 (hardware/video3/docs/plan.md), selected by VIDEO3=1. A SECOND CARD
    ...
    /* audio */
    card_t card;                 /* hardware/audio/refplayer/card.c, stepped per colour clock */
    ...
    /* PS/2 - hardware/io/ps2/docs/ps2.md 8; the ports and devices are ps2_* below */
    uint8_t ioctrl;
...
/* ===================== video3 =====================================
 *
 * hardware/video3/docs/plan.md. What differs from `video`, and nothing else does:
...
static void v3_copy(void)
{
     ...
     * hardware/video3/logic/timing.check.ts is the arithmetic; COPY_PS agrees with
...
/* The audio card is hardware/audio/refplayer/card.c - the register-level model its
...
/* ----------------------------------------------------------------- storage
 * The storage card at $FF58-$FF5B (hardware/storage/docs/sdcard.md §6.2) and an SDHC
 * card in SPI mode behind it, backed by a file.
 *
 * ⚠ THIS IS A PORT, NOT A DESIGN. Every behaviour below is
 * hardware/storage/sim/storage_card.v's or sd_model.v's, which storage_tb.sv
...
/* ------------------------------------------------------------------ PS/2 */
/* hardware/io/ps2/docs/ps2.md, port 0 the keyboard and port 1 the mouse.
 *
 ...
 * software/nitros9/docs/nitros9-av-plan.md 3.4's "IRQ-masked time per primitive". */
...
static void raster(void)
{
    while (m->dots >= m->line_start + DOTS_LINE) {
        ...
        if (m->line == act) {                 /* VBLANK rises */
             ...
             * (hardware/archive/video/logic/sync.timing.ts). demo_tb measured the 12. */
...
int main(int argc, char **argv)
{
    ...
    if (m->v3) fprintf(stderr, "emu: ⭐ VIDEO3 - hardware/video3/docs/plan.md's card, not video/'s\n");
