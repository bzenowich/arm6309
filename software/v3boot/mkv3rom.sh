...
# simulation loads.
#
#   sh software/v3boot/mkv3rom.sh         (from anywhere)
#
# Three outputs, in software/v3boot/:
...
log=$(mktemp)
trap 'rm -f "$log"' EXIT
"$A09" -B"$OUT/v3boot.bin" -L"$OUT/v3boot.lst" "$OUT/v3boot.asm" 2>&1 | tee "$log"
