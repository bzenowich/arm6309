...
**This is a plan, not a specification.** It says what driver code turns `software/archive/demo/`
...
It is based on a read of `hardware/archive/video/docs/graphics.md` and `features.md`, `hardware/audio/docs/audio.md`
and `modplayer.md`, `docs/machine.md`, `hardware/io/ps2/docs/ps2.md`, the demo sources (`demo.asm`,
...
Details are in the research notes; line numbers are from `software/archive/demo/`.
...
- **Register timing is set by the card's timer, not by the CPU.** That is better than the
  demo, whose notes start up to 4 ms late (`software/archive/demo/README.md`, "Why refplayer is not
...
| X7 | ✅ **landed 2026-09-14**: MMU tasks, register-file read-back, the 16C550, VBL 10 lines in the 525 family, `WADV 10`, PS/2 (the card and both devices, checked by `ps2tst`), and the audio card as `hardware/audio/refplayer/card.c`. ⚠ `ASTAT` b6 (the host port's busy bit) is never set, because `card.c` does not model the slot walk. A driver's poll loop is exercised only on `modplay_tb` and the machine | | | every driver iteration; the machine run is 4.5 h | audio.md §16 item 14, g.md §19 item 13 |
...
1. **Armed `GO`** (§3.4.1c). ⭐ **Built 2026-09-14** (g.md §10.3.1), with a `VMODE` family
   ...
   `software/nitros9/docs/nitros9-hardware-improvements.md`'s "Taken" table.
