...
**Question this answers:** `hardware/storage/docs/sdcard.md` §12 step 0 says *"get a filesystem
...
already exists on paper (`hardware/io/serial/`), the software does not, and §7 is the order.

> Superseded material is archived in [history.md](../../../docs/history.md); this document describes
...
| **Hardware cost** | **zero ICs.** It is the I/O card's serial half (`hardware/io/serial/docs/serial.md`), a null-modem cable, and nothing else |
...
`hardware/mainboard/docs/ram.md` §6.7) holding the boot monitor **and a read-only ROM disk large
...
**This machine has no real-time clock.** Not on the motherboard (`hardware/mainboard/docs/ram.md` §6.7
...
`hardware/cpu/docs/plan.md` §2.6's pinout and no crystal for it. **Every boot starts at an
...
- **A terminal is not needed** — this machine has PS/2 and a 640×200 display of its
  own (`hardware/io/ps2/`, `video/`), and `graphics.md` §6.4.8's cell mode is a console at
  1 write per cell.
- **A printer is not needed** by anything.
- ⚠ **A TCP socket is genuinely tempting and it is the wrong answer here**, because
  `hardware/net/docs/net.md` is a real 10BASE-T card with a real stack problem
...
7. **⚠ The virtual TCP socket of §5 would undercut `hardware/net/`.** Recorded as a decision
...
| | |
|---|---|
| [`hardware/io/serial/docs/serial.md`](../../../hardware/io/serial/docs/serial.md) | §4.5 and §5.4 — the `16C550` this document depends on; §8 the level shifter; §12 the second-port question §5 answers |
| [`hardware/storage/docs/sdcard.md`](../../../hardware/storage/docs/sdcard.md) | §11.3, which priced DriveWire as a storage option and rejected it correctly; §12 step 0, which this document replaces |
| [`docs/machine.md`](../../../docs/machine.md) | §7.2 the boot ROM that ended the circularity; §4 the interrupt table §2.1 reads; §6 the open tier decision |
| [`hardware/mainboard/docs/ram.md`](../../../hardware/mainboard/docs/ram.md) | §6.4 the boot path, and §6.7 the ROM |
| [`hardware/io/ps2/docs/ps2.md`](../../../hardware/io/ps2/docs/ps2.md) | §7's host-to-device transmit, which is the hazard in §2.1 |
