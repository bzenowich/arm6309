...
> ([`../archive/README.md`](../../../hardware/archive/README.md), [`history.md`](../../../docs/history.md)). The
> machine's video card is [`hardware/video3/`](../../../hardware/video3/), which already answers several of
...
> NitrOS-9 driver, and `hardware/video3/docs/plan.md` §11 records what it borrowed. ⚠ Every
...
writing the NitrOS-9 video and audio drivers (`software/nitros9/docs/nitros9-av-plan.md`). Each item says
...
| **6** | **H15, hardware sprites** (H1 generalised) | the overworld's hero: 15 tiles the CPU composites, **14 updates a second** (2026-09-17: 4.4 before the software was rewritten — `hardware/video3/docs/demo-report.md` §10.3), and a 0.47–0.78 ms flip in a 1.56 ms blank | **≈ +8 or more**: a compare and a shape store per sprite, and a priority mux in the pixel path | the same timing objection, larger |
...
(`software/archive/demo/bench/vramrate.asm`). **Four things changed, and none of them is
a fit.**
...
| H15 | **A moving figure on a tile screen is fifteen tiles the CPU composites**: each cell under the hero is the world's tile read back through `VDATA` with the sprite's bytes laid over it, written to a spare tile code, and the map cells swapped in the blank (`overworld.asm`, as `demo.asm` does it) | 64 reads and 64 writes a tile. ⭐ **Re-measured 2026-09-17, after the software was rewritten** (`hardware/video3/docs/demo-report.md` §10.3): a hero every **5 frames — 14 a second** at 70 Hz, where it was every 16 and 4.4 a second, and the bare-metal `demo.asm` manages 4 frames. Of the CPU, 34 % was the composite, 18 % `libvid` and **40 % the kernel and I/O stack**; the frame count moved into `+$1F` to stop the loop paying ~1.4 ms a system call to read it. The swap is an `SS.Batch` of up to 30 map writes, **0.47–0.78 ms of a 1.56 ms blank**, measured, with the IRQ at 1.40 ms | **Hardware sprites**: H1's overlay, a few of them and larger, over cell mode as well as bitmap. The figure is a position and a frame, and the map never changes. ⭐ **Or a keyed copyrect** — `CCTRL` b1, skip the write when the byte is the key — which is ~18 cells and 20 pins of `v3ptr`'s spare and needs a fit: **a hero every 1–2 frames** in tile mode (a tile is a 64-byte run, not a rectangle on the stride, so it is ~95 copies), and one copy for the whole figure in bitmap mode, which the scrolling playfield refuses. The arithmetic, including what page-flips in 512 KB, is `demo-report.md` §10.5 |
