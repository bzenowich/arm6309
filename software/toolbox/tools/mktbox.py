...
writes what it draws WITH, as the pages after it:
  ...
  then            Noto Sans 12 px, regular and bold, two bits a pixel
                  the icons, show.Icon blobs - software/archive/demo's, re-quantised
...
DEMO = os.path.join(ROOT, "software", "tools")
PAGE = 8192
FONT_DIR = "/usr/share/fonts/truetype/noto"

# ------------------------------------------------------------------ palette
# 0-15: Haiku's desktop colours (software/tools/show.py GUI16) and KEY.
UI = [
...
# on the most-drawn text in the GUI. software/toolbox/docs/proportional-font.md §4.4.
...
def font_blob(path, size=12):
    ...
    (software/toolbox/docs/proportional-font.md §4). It is baked here rather than derived in the
...
def art_from_rgba(img, outline=True):
    """software/tools/mkshow.py's, against this palette."""
...
# The photograph software/archive/demo's desktop show was made from (git 790de82:
# build/parrots-image.jpg, kept untracked); without it, mkparrots.py's
# stand-in illustration.
PHOTO = os.path.join(ROOT, "software", "toolbox", "reference", "parrots-image.jpg")
...
def text_width(s, bold=False):
    ...
    return sum(glyph_widths(bold)[max(0, min(94, ord(c) - 32))] for c in s)


def glyph_widths(bold=False):
    """The 95 advance widths, codes 32-126, exactly as font_blob bakes them
    into the ROM - which is what tbox.asm's Glyph reads and TextW sums."""
    _, f = font_blob(os.path.join(FONT_DIR, "NotoSans-Bold.ttf" if bold else "NotoSans-Regular.ttf"))
    return [max(1, int(round(f.getlength(chr(c))))) for c in range(32, 127)]


def widths_fcb(bold=True):
    """glyph_widths as assembler, for pasting into a program that has to lay
    text out for itself.  ⭐ desk.asm carries the BOLD face's copy so it can
    work out how wide tbox made its window tab - nothing hands that back, and
    the drag needs it to know which part of the tab band is window and which
    is desktop (software/desk/docs/boot-and-desktop.md §3.7).  The same pattern v3drag uses
    for mklegs.py's table: generated, pasted, and asserted by a bench so the
    copy cannot drift from the ROM's."""
    w = glyph_widths(bold)
    out = ["* GENERATED by `python3 software/toolbox/tools/mktbox.py --widths`.",
           "* ⚠ Do not hand-edit: software/desk/bench/checkmove.py recomputes these from",
           "* the same font this ROM is built with and FAILS if they differ.",
           "TBW.First           equ       32        codes 32-126, as font_blob bakes them",
           "TBW.Count           equ       %d" % len(w),
           "TBWTab              equ       *"]
    for i in range(0, len(w), 12):
        out.append("                    fcb       " + ",".join(str(x) for x in w[i:i + 12]))
    return "\n".join(out)
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--widths":
        print(widths_fcb(bold=True))
    else:
        build(sys.argv[1])
