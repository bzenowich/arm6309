...
#
#   sh software/toolbox/bench/run-v3text.sh          OUT=dir overrides
#
# ⭐ THE FIRST MEASUREMENT OF THE TOOLBOX IN THIS REPOSITORY.  Everything about
# tbox.asm's speed was arithmetic until this: hardware/video3/bench/README.md's "⛔ Does
...
cd "$(dirname "$0")/../../.."
ROOT=$(pwd)
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3text}
...
sh software/nitros9/mkrom.sh "$OUT" > "$OUT/mkrom.log" 2>&1 || { tail -20 "$OUT/mkrom.log"; echo "FAIL  the ROM did not build"; exit 1; }
...
python3 software/toolbox/bench/checkv3text.py "$OUT"
