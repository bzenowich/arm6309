...
`gal/cpld/*.fit` is, and this script re-derives it when the book is present.
...
BOOK = os.path.join(ROOT, "reference", "manuals", "The 6309 Book (Burke & Burke).pdf")
...
def emit_header(path, items, only):
    ...
    print("wrote %s (%d 6309-only encodings)" % (path, len(names)))
