...
#
#   sh software/demo/emu/test/run.sh
#   SEEDS=8 CYC=3000000 SECS=40 ITRACE=1 DEMO=software/demo sh software/demo/emu/test/run.sh
#
...
T=$(pwd)
ROOT=$(cd ../../../.. && pwd)
DEMO=${DEMO:-$ROOT/software/demo}       # another checkout's software/demo, if this one is mid-edit
SEEDS=${SEEDS:-8}
CYC=${CYC:-3000000}
SECS=${SECS:-40}
ITRACE=${ITRACE:-1}
...
CORE="$ROOT/hardware/vendor/mc6809/mc6809e.v $ROOT/hardware/vendor/mc6809/mc6809i.v"
...
if [ ! -f "$DEMO/build/rom-replay.bin" ] || [ ! -f "$DEMO/build/demo.mod" ]; then
  sh "$DEMO/build.sh" || { say "FAIL  software/demo/build.sh"; exit 1; }
fi
"$ROOT/build-host/refplayer" --trace "$O/ref.trace" --sram "$O/ref.sram" --seconds "$SECS" "$DEMO/build/demo.mod" > /dev/null ||
