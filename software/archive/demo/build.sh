...
#
#   sh software/archive/demo/build.sh            (from anywhere)
#
...
# hardware/audio/refplayer/period_table.c.
set -e
cd "$(dirname "$0")"
ROOT=$(cd ../../.. && pwd)
B=build
...
A09="${A09_DIR:-$ROOT/.tools/a09}/a09"

# MOD=path.mod builds the ROM around another module (it is copied to demo.mod)
if [ -n "$MOD" ]; then cp "$MOD" $B/demo.mod; fi
# supplied inputs live in reference/ since 2026-09-23 (a build directory is not
# where something that cannot be regenerated should be kept)
[ -f $B/demo.mod ] || cp reference/demo.mod $B/demo.mod 2>/dev/null || python3 tools/mkmod.py $B/demo.mod
[ -f $B/parrots-image.jpg ] || cp "$ROOT/software/toolbox/reference/parrots-image.jpg" $B/ 2>/dev/null || true
[ -f $B/parrots-image.jpg ] || [ -f $B/parrots.png ] || python3 "$ROOT/software/tools/mkparrots.py" $B/parrots.raw $B/parrots.png
python3 "$ROOT/software/zelda/bench/mkgame.py" $B
python3 "$ROOT/software/tools/mkshow.py" $B
