# `software/archive/demo/` — a desktop, a paint program, a BBS and an overworld, on the whole machine
...
[`hardware/archive/video/docs/features.md`](../../../hardware/archive/video/docs/features.md) §0, as a user would see it: a
...
```sh
sh software/archive/demo/build.sh                      # assets, script, layout, assemble -> build/rom.hex
sh software/emu/run-emu.sh 120            # the show on the host emulator, in seconds
VIDEO=1 sh software/emu/run-emu.sh 120    # ... and /tmp/arm6309-emu/emu.mp4, with sound
python3 software/archive/demo/tools/checkdemo.py /tmp/arm6309-emu software/demo/build
...
sh software/archive/demo/bench/run-replay.sh 40        # the replayer vs refplayer, CPU alone (~2 min)
sh software/archive/demo/bench/run-calib.sh            # bench/calib.asm on the whole machine (~15 min)
```
...
| `replay.asm` | the module loader and replayer — `hardware/audio/refplayer/mod_replay.c`, transliterated routine for routine |
...
| `emu/ps2script.h` | ⭐ **`PS2_SCRIPT`**: a timed, semantic keyboard and mouse script — `move to X Y`, `click left`, `type "..."` — encoded into set-2 scan codes and 9-bit-split mouse packets and fed to `machine.c`'s line-level PS/2 devices. `hardware/io/ps2/docs/ps2.md` §11.5 |
...
MOD=path/to/song.mod sh software/archive/demo/build.sh         # the module (it is copied to demo.mod)
```
