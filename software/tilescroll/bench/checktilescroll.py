#!/usr/bin/env python3
"""checktilescroll.py DIR [FRAMES] - the streaming engine's ring, byte for byte.

    python3 software/tilescroll/bench/checktilescroll.py software/tilescroll/build/scroll 1200
...
sys.path.insert(0, str(ROOT / "software" / "tilescroll" / "bench"))
import mktilescroll as S                                        # noqa: E402

# ⭐ The camera's path, in 8.8 PIXELS A FRAME, and it has to agree with
# tilescroll.asm's PathTab exactly - including the 362 that makes a diagonal leg
...
def main(d, frames):
    ...
    # the copy engine's key and mktilescroll refuses it in every terrain tile, so a
