#!/usr/bin/env python3
"""mktilescroll.py [CMDSDIR] - a 32 x 32 TILE BANK, a world map, and the equates.

    python3 software/tilescroll/bench/mktilescroll.py ../nitros9/level2/arm6309/cmds
...
The files:
    ...
    tilescroll.bnk    288 x 512 bytes - 9 columns x 16 rows of 32 x 32 tiles
    scroll.art    the keyed actor bank, one 32-row strip
    tilescrolldat.asm every equate the program and this file share, and the map
"""
import pathlib
import sys
...
# ⛔ THE GEOMETRY.  Every number here is in tilescrolldat.asm too; the program
...
def tileset():
    ...
    Layout, and `TileIx` in tilescrolldat.asm is the same table:
...
def bank(tiles):
    ...
    if len(tiles) > ZNTILE:
        sys.exit("FAIL  mktilescroll: %d tiles is more than the bank's %d"
                 % (len(tiles), ZNTILE))
...
# tile is.  ⚠ The cast's shapes (cast.py, drawn for the archived `zelda`) are
...
DAT = """\
********************************************************************
* tilescrolldat.asm - GENERATED by arm6309 software/tilescroll/bench/mktilescroll.py.  Do not edit.
*
...
* has ring slots 22..30 - exactly the 288 columns tilescroll.bnk is wide.
ZBK0                equ       {ZBK0}
...
def main(cmds):
    ...
    b = bank(all_tiles)
    (d / "tilescroll.bnk").write_bytes(b.tobytes())
    ...
    (a / "tilescrolldat.asm").write_text(DAT.format(
    ...
    print("ok    %s/tilescrolldat.asm: map %d x %d tiles, bank at ring slot %d"
