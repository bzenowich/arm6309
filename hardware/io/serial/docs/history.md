...
The video card's VRAM read-back was built prefetched (`hardware/archive/video/docs/graphics.md` §11) and stopped being a reason ÷8 is unsupported.
...
2. **`hardware/storage/docs/sdcard.md` §6.1 returned four bytes** (it needed only half the disk
   reservation), reopening `$FF5C`–`$FF5F`.
3. **`hardware/net/docs/net.md` §5.1 spent those on 2026-09-07** and the map was full again —
...
Replaced by: §7.1's present eight-row map (matching `hardware/tools/lib/windows.ts`) and its
two standing consequences (the seven-bit decode; addresses no longer the scarce
currency). The board merge with PS/2 (2026-09-08, `hardware/io/board/io.circuit.tsx`,
