...
([`drivewire.md`](../../../../software/nitros9/docs/drivewire.md)) as the writable paths.
...
and `hardware/io/README.md`'s test is *"whether a period part exists that fits the I/O budget"*,
...
> ⚠ **This card shares a board with PS/2** — `hardware/io/board/io.circuit.tsx`, 14 ICs on
...
upper half of it. `hardware/tools/lib/windows.ts` is the map as data:
...
| `$FF58`–`$FF5B` | 4 | storage — `hardware/storage/docs/sdcard.md` §6.1 |
| `$FF5C`–`$FF5F` | 4 | net — `hardware/net/docs/net.md` §5.1 |
...
⚠ **The machine gave up eight bytes of margin** — 64 free became 56. `make -C hardware check`
prints the figure and `hardware/tools/lib/cards.check.ts` is what stops two windows overlapping.
...
- **The decode is `A0`–`A6`, seven bits.** `A6` is not implied by the window strobe, so a
  ...
  from this card's decode entirely — `hardware/io/board/io.circuit.tsx` compared `A2`–`A5`
...
are one **I/O card of 14 ICs** now (`hardware/tools/place/parts.ts`, `ps2.md` §9 + §9 here).
...
(`hardware/io/board/io.circuit.tsx`) and they split the work: the serial one compares the
...
including a wiring defect it found in `hardware/io/board/io.circuit.tsx`.
...
| GAL22V10 | 1986 | in period; the machine's placement carries five of them (`hardware/tools/place/parts.ts`) |
...
[`TL16C550C.pdf`](../reference/TL16C550C.pdf), and a fourth thing turned
...
⚠ **And nothing could have caught it.** `make -C hardware netlist` reads the *motherboard's*
...
4. **⚠ NEITHER OF THE CARD'S GALs IS FITTED, and they have more to carry than they did.**
   ...
   `hardware/io/board/io.circuit.tsx` entirely until 2026-09-09. It is on the serial GAL
   ...
   card's fitting risk. **Fit both before laying out the board** — `tools/gal/README.md` lists
...
10. **One port, and a second is affordable — but DriveWire no longer needs it.** **The
   ...
   DriveWire link on the other"* — and [`drivewire.md`](../../../../software/nitros9/docs/drivewire.md) §5
...
| [`drivewire.md`](../../../../software/nitros9/docs/drivewire.md) | **the strongest single argument for §4.5's tier** — §3 there prices DriveWire at 1.8 KiB/s and 37 % of the CPU on the 6551 against 11.0 KiB/s and 16 % on a `16C550`; §2.1 is the PS/2 collision from this card's item 11, one part further along |
