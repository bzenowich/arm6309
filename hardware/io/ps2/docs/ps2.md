...
| Burrell Smith's two-chip Apple II mouse card; the interrupt-per-notch model; the video-sync flip-flop | Andy Hertzfeld, *Apple II Mouse Card*, folklore.org, June 1981 — `hardware/io/ps2/reference/folklore-apple2-mouse-card.pdf` | **read directly**; a first-hand account, but a *narrative*, not a schematic |
...
([`software/nitros9/docs/nitros9-hardware-improvements.md`](../../../../software/nitros9/docs/nitros9-hardware-improvements.md)
...
The geographic decode spans `$FF00`–`$FF7F` (`machine.md` §2; `hardware/tools/lib/windows.ts`
...
> `hardware/io/board/io.circuit.tsx` compared `A2`–`A5` and nothing else, which is the
...
> `hardware/io/board/io.circuit.tsx`, 14 ICs on a 12 cm card. The card's logic, its
...
Andy Hertzfeld's account (`hardware/io/ps2/reference/folklore-apple2-mouse-card.pdf`, June 1981)
...
(`software/desk/docs/boot-and-desktop.md` §3) — and the script is what clicks at it. The host emulator
takes a **timed, semantic input script**:
`PS2_SCRIPT=file` in `software/emu/machine.c`, with the format, the scan code table
and the encoding in [`software/emu/ps2script.h`](../../../../software/emu/ps2script.h).
...
`software/emu/test/run-ps2script.sh` is the bench: it encodes each script a second
...
`software/desk/bench/run-v3desk.sh` runs the desktop shell off the SD card and clicks at its
menu bar (`software/desk/docs/boot-and-desktop.md` §3). It is the one that exercises the *driver* -
...
| # | Step | Exit criterion |
|---|---|---|
| 0 | **Settle `machine.md` §5 items 1 and 2** — `$FF30`–`$FF33` (moved from `$FF30` 2026-09-09, §3.2), and `/IRQ` as an added source | `machine.md` records both as taken rather than proposed (`hardware/tools/lib/windows.ts` still carries the window as proposed) |
...
2. **`$FF30`–`$FF33` is still a proposal** (§3.2) — `hardware/tools/lib/windows.ts` carries
...
| **Andy Hertzfeld, *Apple II Mouse Card*, folklore.org, June 1981** | Burrell Smith's two-chip design. §4.4 takes the interrupt argument and the derive-it-from-the-bus habit, and rejects the interrupt-per-notch mouse on §4.4(c)'s arithmetic. `hardware/io/ps2/reference/folklore-apple2-mouse-card.pdf` |
