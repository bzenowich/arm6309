...
Replaced by: `ps2.md` §9's flat count of 11, confirmed by `hardware/tools/place/parts.ts`
...
3. **"A quarter of the disk controller's window, not half."** The section title and body
   ...
   Replaced by: the present map in `ps2.md` §3.2 / `hardware/tools/lib/windows.ts`.
...
The PS/2 and serial cards merged onto one board — `hardware/io/board/io.circuit.tsx`, 14 ICs
...
## hardware/io/README.md — the `$FF` map chain and the 9 → 11 warning block
...
`software/nitros9/docs/nitros9-hardware-improvements.md` H11). Replaced by: `KIRQEN` (b6) and `MIRQEN`
...
`software/emu/ps2script.h`'s `G` line was fed from **every** load of `MDATA`.
...
It was found by `software/desk/bench/run-v3desk.sh` on its first run, where the reported pointer
