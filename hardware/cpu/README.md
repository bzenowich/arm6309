# `hardware/cpu/` — the HD6309E module
...
[`hardware/archive/video/docs/graphics.md`](../archive/video/docs/graphics.md) §6.3.1, and `docs/plan.md` §3.2 for
...
figure (`hardware/cpu/reference/HD6309E_datasheet.pdf` p.3, held at 110 ns for both the
...
`hardware/cpu/src/spike_poll_asm.S` unrolls by two and alternates destination registers, removing the
...
**`hardware/mainboard/docs/ram.md` §5.2 re-carved the map to 32 MB and `machine.md` §7.2 put a 1 MB ROM
...
```sh
make check          # from this directory: cmake into build/host, then ctest
```
...
make firmware       # from this directory
```

Produces `build/arm/spike.elf`, `.bin`, `.hex` and a link map. This directory is
a CMake project of its own (`CMakeLists.txt`); the toolchain file is
[`cmake/arm-none-eabi.cmake`](cmake/arm-none-eabi.cmake).
...
1. Wire the stimulus generator to E, Q and D0–D7 — see
   [`hardware/cpu/tools/stimulus/README.md`](tools/stimulus/README.md).
...
The number it produces decides whether the pipelined architecture (`hardware/cpu/docs/plan.md` §4.2)
...
Paths are relative to `hardware/cpu/`. Build commands run from the repository root.
...
- [x] Variant 2 — hand-written assembly (`hardware/cpu/src/spike_poll_asm.S`), `T_iter` 4/6 cycles
- [x] Variant 3 — DMA-driven address (`hardware/cpu/src/spike_dma.c`)
...
- Variant 4 does not exist — the 200 MHz overclock is rejected (`hardware/cpu/docs/plan.md`
...
- [ ] First silicon measurement; record actuals against predictions in `hardware/cpu/docs/plan.md` §5
