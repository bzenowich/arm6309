...
> VRAM read-back does not close (`hardware/archive/video/docs/graphics.md` §11), a 2 MHz 6551 is 57 % over
...
took, at three packages and a megabyte, because `hardware/mainboard/docs/ram.md` §5.2's 32 MB re-carve
...
>   machines.
>
> > ⚠ **Consequence for the drop-in claim.** `hardware/archive/video/docs/graphics.md` §16.8's "you can
...
> peripheral set, same 42 GPIO the tables assumed — so the pinout, `hardware/cpu/include/pinout.h`
...
2026-09-04.**"), the README a longer restatement, and `hardware/cpu/include/pinout.h` still
...
> address deadline. That was wrong. The replacement used `hardware/cpu/reference/MC6809E.pdf`,
...
> is now in the repo: `hardware/cpu/reference/HD6309E_datasheet.pdf` p.3 gives AC
...
`hardware/cpu/tools/microstate-probe/` measured it at **~80 core cycles per bus cycle — over
...
(`hardware/mainboard/docs/ram.md`), which changes nothing about D1.
...
`hardware/cpu/include/spike.h`). The bias analysis and the calibration path back to 18 are
...
|---|---|---|
| Microcode step costs more than 15–30 cycles | **Largely closed** | Measured: 14 cy typical, 25 cy worst, against 34 available at 3 MHz (§3.3(d), `hardware/cpu/tools/microstate-probe/`). Residual coverage risk lives on in the lazy-CC and probe-coverage rows. |
...
- **Item 3 (`t_DSR`/`t_DHR`?)** was answered twice: first from
  `hardware/cpu/reference/MC6809E.pdf` (40 ns / 10 ns, MC68B09E), then properly from the
...
> MC6809E datasheet (`hardware/cpu/reference/MC6809E.pdf` p.3, item 11), *Address Delay
