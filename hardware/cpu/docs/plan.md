...
> below, `hardware/cpu/include/pinout.h` and the firmware work verbatim; QFN soldering is the
...
**The authoritative timing source is `hardware/cpu/reference/HD6309E_datasheet.pdf`.**
...
**Datasheet timing — `hardware/cpu/reference/HD6309E_datasheet.pdf` p.3, "AC CHARACTERISTICS".**
...
Also worth recording, since `hardware/cpu/reference/MC6809E.pdf` disagrees: Motorola specifies `t_DHR` as
...
`hardware/cpu/tools/microstate-probe/` compiles candidate microstates with the firmware's exact flags
...
Component costs, tuned hot path (`hardware/cpu/tools/microstate-probe/tuned.S`):
...
`hardware/cpu/tools/microstate-probe/README.md`). Treat fast-E as unproven on throughput as well
...
video card's VRAM read-back no longer does: it is prefetched, `hardware/archive/video/docs/graphics.md` §11.) The CPU module's
...
3. **Hardware-assisted address drive** (implemented as spike variant 3,
   `hardware/cpu/src/spike_dma.c`). The E falling edge triggers a DMA transfer of a *precomputed*
...
4. **Overclock the G4 to 200 MHz — evaluated and rejected (2026-08-28).**
   ...
   **You cannot overclock the core without also overclocking the PLL VCO, and there is
   ...
   is 344 / 2 = 172 MHz.** `hardware/cpu/src/clock.c` already runs the VCO at 340 MHz — *98.8% of the
...
- The CoCo 3 is 5 V; the G4 is 3.3 V, and **most pins this design needs are not
  ...
  list is mirrored in `hardware/cpu/include/pinout.h`.
...
**DS12589 Table 12 (`hardware/cpu/reference/stm32g431kb.pdf` pp.52–53) lists `PA0`–`PA7` as `TT_a`, every
...
every target rate, including 1.79 MHz** (`hardware/cpu/tools/microstate-probe/`). Two indirect
...
this core that is a choice rather than a bug. `hardware/cpu/docs/6309.md` §5.1 is the full
argument; `hardware/storage/docs/sdcard.md` §11.6 priced it. In short:
...
- ⭐ **It is unobservable to NitrOS-9.** All 126 `TFM` sites in the OS are either
  ...
  (`hardware/cpu/docs/6309.md` §5.1.5 has the census).
...
**`hardware/mainboard/docs/ram.md` §5.2 re-carved the map to 32 MB on 2026-09-08 and
...
| ⭐ **The drop-in claim** | `hardware/archive/video/docs/graphics.md` §16 item 8's *"you can drop a real HD63C09E into the homebrew machine"* **holds again** — the machine's boot ROM is on the bus, so a part with no flash boots from it |
...
`hardware/mainboard/docs/ram.md` §6.4 — proposed serving 2 KB of STM32 SRAM as a boot scratch window.
...
- Deadline misses detected in-loop against a **hardware TIM1 timestamp** of E-fall:
  ...
  > (`hardware/cpu/tools/stimulus/README.md`), so the `'541` propagation outbound and the `'541`
  ...
  >
  > **The gate is 14** (`hardware/cpu/include/spike.h`, `SPIKE_TAD_CYCLES`; the raw 18 is kept
...
3. *Answered* — `t_DSR` 40 ns (B grade) / **20 ns** (C grade), `t_DHR` 20 ns Hitachi /
   ...
   (`hardware/cpu/reference/HD6309E_datasheet.pdf` p.3; §3.3).
...
3d. *Answered* — yes, with the §4.1 rules: measured 14 cycles typical / 25 worst against
   34 available at 3 MHz (`hardware/cpu/tools/microstate-probe/`, §3.3(d)). The naive shape costs
...
- **HD63B09E / HD63C09E datasheet** — `hardware/cpu/reference/HD6309E_datasheet.pdf`. **The authoritative
  bus-timing source for this project** (p.3, AC characteristics, both grades in adjacent
  columns). Supersedes `hardware/cpu/reference/MC6809E.pdf` for everything in §3.3.
- STM32G431CB datasheet DS12589 Rev 6 — `hardware/cpu/reference/stm32g431kb.pdf`. Table 45 (PLL
...
- **RM0440 Rev 9** — `hardware/cpu/reference/stm32g4-refman.pdf`. §9.3.1 (GPIO `IDR` free-running),
  Table 2 §1.6 (no FSMC/QUADSPI on G431), §13.4.4–13.4.5 (DMAMUX sync/request generator).
  Sources for §3.4(6).
- **SAM D5x/E5x family datasheet** — `hardware/cpu/reference/ATSAMD51G19A.pdf`. §52 (PCC), §32 (PORT behind
...
- MC6809E datasheet — `hardware/cpu/reference/MC6809E.pdf`. Retained for the Motorola part actually in the
