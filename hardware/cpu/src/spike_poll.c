/* Phase 1 timing spike, variant 1: polling loop in C.  hardware/cpu/docs/plan.md §5.
 *
...
__hot void spike_run_poll(uint32_t n_cycles, spike_result_t *out)
{
    ...
    for (uint32_t i = 0; i < n_cycles; i++) {
         ...
         * (hardware/cpu/docs/plan.md §2.1): if the clock speeds up, Q simply falls sooner
         ...
         * switch (hardware/cpu/docs/plan.md §2.1) does not relax it. t_AH is the floor:
