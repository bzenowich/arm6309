...
- `hardware/mainboard/logic/mmu.pld`'s entire design rests on the **E/Q quadrature** — the phase table
  in `graphics.md` §6.3.1, and the 174 ns break-before-make margin derived in
  `hardware/tools/gal/README.md`. A core that gets the instruction set right and the cycle
  timing wrong cannot check any of it.
- `mc6809e.v`'s ports **are** the 40-pin socket in `hardware/tools/lib/parts.ts`, one for one,
  and it takes `E` and `Q` as *inputs* — so a testbench supplies the quadrature exactly as
  `mainboard/sim/mmu_tb.sv` already does.
- It was validated against a **TRS-80 Color Computer 3**, among others. That is this
  project's other target machine (`hardware/cpu/docs/plan.md`).

**And the instruction set is not what is missing.** The CPU in this machine is an
STM32G431 emulating an HD6309E (`hardware/cpu/`), and what reaches the motherboard is a **6809E
...
`mainboard/sim/mmu.v` and a model of U1/U2/U4/U5, so the map-write cycle is exercised by a CPU
