...
3. **Runtime speed switching**, 0.895 ↔ 1.79 MHz, changing between one cycle and the
   ...
   `hardware/cpu/docs/plan.md` §2.1 — software on a CoCo 3 flips speed by poking `$FFD9`, and the bus
   ...
   (`SPIKE_TAD_CYCLES` in `hardware/cpu/include/spike.h`), which is correct precisely *because*
...
74LVC level-shifting described in `hardware/cpu/docs/plan.md` §3.5 belongs to Phase 7, on the real
