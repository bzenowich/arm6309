/* Phase 1 timing spike — measurement API.  See hardware/cpu/docs/plan.md §5.
 *
 ...
 *   > hardware/cpu/docs/plan.md §3.3, which corrects both.
 *
 ...
 * hardware/cpu/docs/plan.md §3.3 estimates at 4-6 core cycles and expects to dominate the
 ...
 *   3. Phase 1 has no buffers (hardware/cpu/tools/stimulus/README.md), so the '541
 ...
 * CHARACTERISTICS" table, 2 MHz column (hardware/cpu/reference/MC6809E.pdf p.3). One TIM1 tick
...
/* Variant 1 (hardware/cpu/docs/plan.md §5): polling loop written in C.
...
/* Variant 2: the same loop hand-written in assembly (hardware/cpu/src/spike_poll_asm.S).
 *
 ...
 * drive path at all (hardware/cpu/src/spike_dma.c).
 *
