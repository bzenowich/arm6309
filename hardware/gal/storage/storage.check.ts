 ...
 *   bun run gal/storage/storage.check.ts        (part of `npm run check`)
 *
 ...
 *
 * ⚠ Both parts also go through Atmel's CUPL (jedec/reference/*.cupl.jed) -
...
for (const { d, gal } of [{ d: sdbusDesign, gal: B.gal }, { d: sdengDesign, gal: E.gal }]) {
  const ref = join(here, "..", "jedec", "reference", `${d.name}.cupl.jed`)
  if (!existsSync(ref)) {
    check(false, `CUPL's ${d.name}.cupl.jed exists`,
      `run gal/prjbureau/cupl-reference.sh gal/storage/${d.name}.pld`)
    continue
  }
