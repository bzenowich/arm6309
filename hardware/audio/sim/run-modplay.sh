...
#
#   sh audio/sim/run-modplay.sh song.mod [seconds]      (from hardware/)
...
# cycles; the four AD7528 pairs' codes come out; hardware/audio/tools/dacwav renders
# those through render.c's analogue chain, which is resistors and an op-amp and
# has no RTL to have.
#
# ⛔ WHAT IS NOT: hardware/audio/refplayer/card.c, the host-side model of this card. It
...
SECS=${2:-4}
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/modplay}
mkdir -p "$OUT"
name=$(basename "$MOD" .mod)

REF="$ROOT/hardware/audio/build/host/refplayer"
DACWAV="$ROOT/hardware/audio/build/host/dacwav"
[ -x "$REF" ] || { echo "hardware/audio/build/host/refplayer missing - make -C hardware/audio host" >&2; exit 1; }
[ -x "$DACWAV" ] || { echo "hardware/audio/build/host/dacwav missing - make -C hardware/audio host" >&2; exit 1; }
...
python3 -c "import numpy, sys; sys.path.insert(0, '$ROOT/hardware/audio/tools/modcompare'); import omptrender; omptrender._lib()" \
...
# and fails too.
python3 "$ROOT/hardware/audio/tools/modcompare/abcompare.py" "$MOD" --seconds "$SECS" \
