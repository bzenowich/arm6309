...
#
#   sh audio/sim/probes.sh          (from hardware/)   -> prints the directory
#
...
# SYNTHESISED - hardware/audio/tools/modcompare/mkprobe.py writes its own saw and square
...
DIR=${ARM6309_MOD:-$ROOT/hardware/audio/build/mod}
...
if [ ! -f "$DIR/14_fourchan.mod" ]; then
  ...
  python3 "$ROOT/hardware/audio/tools/modcompare/mkprobe.py" "$DIR" >&2
fi
