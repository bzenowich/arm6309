...
import { counterTerms } from "../../tools/gal/jedec/counter"
import { reduceTerms } from "../../tools/gal/jedec/minimise"
...
const sources: Cell[] = [
  ...
  /* ⛔ AND ON THE LAST STEP OF THIS CHANNEL'S OWN W6, because every compare
   * hit before it was against a STALE deadline (audio.md 16 item 48). NEXT
   * keeps the value it had when the channel was disabled until W6 rewrites it
   * on its last step, and the compare keeps running while the enable edge
   * waits in RST and while W6 runs - so if the counter crosses the old NEXT in
   * that window, a DUE is raised that W6 then serves as an extra W1, a few
   * colour clocks after the chained one: the first sample of the note plays
   * for a few colour clocks instead of PER. It cannot be PICKED earlier -
   * RSTANY outranks DUEANY, and W6 holds BUSY - so clearing it here covers the
   * whole window. item 37's re-timing made W6 eleven steps and tripled that
   * window, and audio_tb's CNT claim after an enable is what saw it. */
  ...[0, 1, 2, 3].map((n) => ({
    pin: 0, name: `CLR${n}`, assertedLow: false, s0: 1 as const, registered: false,
    terms: [`DPICK${n}`,
      `RUN & !WT2 & WT1 & !WT0 & LAST & ${n & 1 ? "" : "!"}WC0 & ${n & 2 ? "" : "!"}WC1`],
  })),
