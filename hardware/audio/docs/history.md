...
**`hardware/audio/logic/aseq.micro.ts` — W4, the ten steps that were deleted:**
...
>     write pulse. [`AD7528.pdf`](../reference/AD7528.pdf), `VDD` = +5 V:
>
>     | | 25 °C | −40…85 °C |
>     |---|---|---|
...
gain any that `audio/logic/cpld/aseq.fit` can show"*. Its "what is left" table carried
...
footprint and `hardware/tools/lib/parts.ts` entry were left with nothing citing them. **The
...
and `hardware/tools/lib/slot.ts` carried them at B32–B35 — and **nothing in the machine
...
format is 100 mm × 120/180/240 mm per card, and `hardware/tools/place/` puts this card on
...
`hardware/audio/logic/audio.jedec.ts`, both refusals asserted by `check:audio`), which took
...
`tools/place/parts.ts` has carried **four** since the datapath was built, and the parts list
...
⛔ **And it is the class `tools/lib/docs.check.ts` cannot catch.** That check holds utilisation
figures and IC totals against `<card>/logic/cpld/*.fit` and `tools/place/parts.ts`; "two packages of
...
those". [`cd74hc283.pdf`](../reference/cd74hc283.pdf) is now in the
...
`hardware/audio/tools/modcompare/` A/Bs `hardware/audio/refplayer/` — **a C model of this card** — against
...
item 0b, in the tooling this time. [`audio/sim/oracle/`](../sim/oracle/)
...
foldback, one cascade, 433 PT, and `hardware/audio/README.md` quoted 124 / 128 with "four cells
...
### hardware/audio/docs/audio.md — §10 parts list, the U2 row

**the sequencer (§10.2). Fitted at 128 of 128 cells and 62 of 64 I/O** (§10.2.6)

### hardware/audio/docs/audio.md — §10.2.6, the sentence after the W6-into-W1 chain
...
### hardware/audio/docs/audio.md — §10.3, the adder paragraph

What decides is that U2 is at 128 of 128 cells.

### hardware/audio/docs/audio.md — §15 build order, step 4's status
...
### hardware/audio/docs/audio.md — §0, item 37 note
...
### hardware/audio/docs/audio.md — §3.1, the work-slot margin
...
### hardware/audio/docs/audio.md — §10, the parts table note
...
### hardware/audio/docs/audio.md — §10.1, the TQFP-100 sentence

pins from 63 of 64 to 63 of 80 and leaves the cells at 126 of 128, because both packages

### hardware/audio/docs/audio.md — §10.1, nothing left on U2
...
### hardware/audio/docs/audio.md — §10.2.3, the re-timing paragraphs
...
### hardware/audio/docs/audio.md — §10.2.3, the fan-in paragraph

**Seven of U2's eight logic blocks stand at 36 of 40 signals** (`cpld/aseq.fit`), which

### hardware/audio/docs/audio.md — §10.2.5, heading

#### 10.2.5 What it costs in slots — and §3.1's 56× is 46×

### hardware/audio/docs/audio.md — §10.2.5, the slot table and floor
...
### hardware/audio/docs/audio.md — §10.2.5, the measured margins
...
### hardware/audio/docs/audio.md — §10.2.6, the fit paragraph
...
### hardware/audio/docs/audio.md — §10.2.6, the cell history sentence

after §16 item 47's host-strobe repair U2 is at **126 of 128**.

### hardware/audio/docs/audio.md — §10.3.5, the 53 ns sentence
...
### hardware/audio/docs/audio.md — §15, step 4
...
### hardware/audio/docs/audio.md — §16 item 37, the repair paragraph

**229 ns** in §10.2's arrangement, **211 ns** in §10.3's.

### hardware/audio/docs/audio.md — §16 item 37, sub-item 1

1. ⚠ **Over temperature nothing closes** — 240 ns against 229.

### hardware/audio/docs/audio.md — §16 item 37, the 74ACT283 sum

that is **93 ns into the 229 ns

### hardware/audio/docs/audio.md — §16 item 37, the 74ACT283 margin

temperature range with 2.4× margin, and it stays a `74HC` card**

### hardware/audio/docs/audio.md — §16 item 37, sub-item 3
...
### hardware/audio/README.md — the item 37 note, first sentence

datasheet says it is 192 ns into a 53 ns window** (§16 item 37).

### hardware/audio/README.md — the item 37 note, the repair
