/* Tests for the reference module player — hardware/audio/refplayer/.
 *
 * These check the things hardware/audio/docs/modplayer.md §9 says a test corpus must check,
...
static void test_volume_law(void)
{
    ...
    /* hardware/audio/docs/audio.md §6.1: the volume is the CODE of a second multiplying
...
static void test_period_is_the_colour_clock(void)
{
     ...
     * This is hardware/audio/docs/audio.md §4.1 and §4.2 -- if it is wrong, every module is
...
{
    /* hardware/audio/docs/audio.md §3.3 / modplayer.md §5.3: the card must reload from LC/LEN
...
{
    /* hardware/audio/docs/audio.md §8.2. The clock is colourclock/5 and the reload is
...
{
    /* hardware/audio/docs/audio.md §7. Two things must hold, and only the first is a matter of
...
{
    /* hardware/audio/docs/modplayer.md §5.4 / §11 item 4. The table is copied, not computed,
...
{
    /* hardware/audio/docs/audio.md §6.1: the REPLAYER turns Paula's 0..64 into the
