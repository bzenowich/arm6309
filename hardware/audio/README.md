# `hardware/audio/` — the 4-channel PCM card
...
```sh
make host                                   # from this directory -> build/host/
...
[`hardware/audio/sim/modplay_tb.sv`](sim/modplay_tb.sv) uploads a
...
records what the four `AD7528` pairs are given. `hardware/audio/tools/dacwav` renders that
...
([`../hardware/gal/verilog/oracle/`](sim/oracle/), `npm run
