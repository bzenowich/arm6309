/* The ProTracker replayer — hardware/audio/docs/modplayer.md §5.
 *
 ...
 * volume converter's 8-bit code (hardware/audio/docs/audio.md §6.1). 4v, saturated at
...
{
    /* hardware/audio/docs/audio.md §8.2: the timer counts at colourclock/5 — the Amiga's CIA
...
/* hardware/audio/docs/modplayer.md §5.3 — the one sequence that must be exactly right. */
static void commit_triggers(mod_player *p)
{
     ...
     *    one-shot runs out (hardware/audio/docs/audio.md §3.3). ProTracker's delay loop is not
...
static void e_tick0(mod_player *p, unsigned n, uint16_t note)
{
    ...
    case 0x0:
         ...
         * other than a channel register (hardware/audio/docs/modplayer.md §5.6). x=0 turns the
    ...
    case 0x6:
        if (x == 0u) {
            ...
            if (ch->loop_cnt) {
                 ...
                 * processed wins (hardware/audio/docs/modplayer.md §5.8). */
                p->loop_pending = 1;
    ...
    case 0xF:
         ...
         * Unimplemented by decision, not by omission — hardware/audio/docs/modplayer.md §10.8
         * and §11 item 3. */
        break;
...
/* hardware/audio/docs/modplayer.md §5.8. The order of these steps is where players diverge. */
...
void mod_start(mod_player *p, mod_song *s, card_t *c)
{
     ...
     * precisely so this order is possible (hardware/audio/docs/audio.md §9.2). */
    for (unsigned n = 0; n < MOD_CHANNELS; n++) {
...
void mod_tick(mod_player *p)
{
     ...
     * fire once per repeat (hardware/audio/docs/modplayer.md §10.10) -- but it does not
    ...
    /* Sole /FIRQ source, so no polling chain: it was us (hardware/audio/docs/audio.md §8.1). */
