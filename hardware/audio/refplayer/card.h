/* Model of the arm6309 4-channel PCM sound card — hardware/audio/docs/audio.md.
 *
 * This is a REGISTER-LEVEL model, not a mixer. Everything the replayer does
 * goes through card_write()/card_read() at the offsets hardware/audio/docs/audio.md §9.2
 ...
 * can be diffed against the other. That is the whole reason hardware/audio/docs/audio.md §12.5
 ...
 * reference (hardware/audio/docs/audio.md §4.1). Nothing here is resampled, interpolated or
 * approximated: a channel's sample changes on the exact colour clock its period
 * counter fires, which is what hardware/audio/docs/audio.md §6.2 builds the hardware to do.
 *
 * Deliberately NOT modelled: the 8-slot walk, the pipeline stages, and the
 * deferred queue (hardware/audio/docs/audio.md §3). Those are how the hardware finds the time
 * to do this work, not what the work is. The one place slot timing is
 * observable to software is the DMACON-enable latch, which hardware/audio/docs/audio.md §16
 * item 13 requires to be bounded at 4 colour clocks; §5.3 of hardware/audio/docs/modplayer.md
 ...
 *
 * hardware/audio/docs/audio.md §6.1 has specified three different volume paths, and this
...
/* hardware/audio/docs/audio.md §16 item 13: enable-triggered work jumps the deferred queue
