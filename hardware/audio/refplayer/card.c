/* Model of the arm6309 sound card. See card.h and hardware/audio/docs/audio.md. */
...
void card_reset(card_t *c, uint8_t *sram, uint32_t sram_bytes)
{
    ...
    /* hardware/audio/docs/audio.md §13: reset leaves the card quiet — display of the audio
...
void card_write(card_t *c, uint8_t reg, uint8_t val)
{
    ...
    case A_ADATA: {
        ...
        if (off <= ST_PAN) {
             ...
             * (hardware/audio/docs/audio.md §1 requirement 8), so a write there is
            ...
            for (unsigned n = 0; n < 4u; n++) {
                if ((mask >> n) & 1u) {
                    if (!c->ch[n].dmaen) {
                         ...
                         * shadow. hardware/audio/docs/audio.md §16 item 13 bounds the delay,
...
uint8_t card_read(card_t *c, uint8_t reg)
{
    ...
    case A_ADATA: {
         ...
         * so they never stall — hardware/audio/docs/audio.md §9.3. */
        return c->state[i];
    }
    case A_AINTREQ:
        return c->intreq;
...
static void chan_tick(card_t *c, unsigned n)
{
    ...
    if (ch->ptr < c->sram_bytes && c->sram) {
         ...
         * (hardware/audio/docs/modplayer.md §4.6), which is worth failing a test over. */
        byte = 0x80u;                 /* offset-binary silence, §6.1 */
        c->oob_reads++;
    }

    /* No attach modulation: it was retired 2026-09-11 (hardware/audio/docs/audio.md
    ...
    if (--ch->cnt == 0u) {
        /* THE shadow reload — hardware/audio/docs/audio.md §3.3, requirement 3. It takes
...
void card_dac(const card_t *c, int *l, int *r)
{
    ...
    /* Hard-panned, 0 and 3 left, 1 and 2 right — hardware/audio/docs/audio.md §1 requirement
