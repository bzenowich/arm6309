...
**[`hardware/audio/docs/modplayer.md`](../docs/modplayer.md) §8, build step 0.** Everything
...
```
make -C .. host                             # -> hardware/audio/build/host/
...
|---|---|---|
| `card.c` | the sound card, at register level, one colour clock per step | [`audio.md`](../docs/audio.md) |
| `mod_load.c` | the loader — relocation and validation only | [`modplayer.md`](../docs/modplayer.md) §4 |
| `mod_replay.c` | the replayer — the tick engine and the full effect set | [`modplayer.md`](../docs/modplayer.md) §5 |
| `render.c` | the analogue chain, and the decimation that is **not** part of it | [`audio.md`](../docs/audio.md) §6.2, §7 |
...
- **`E3x` glissando** is implemented; **`EFx` invert loop** is not, by decision
  ([`modplayer.md`](../docs/modplayer.md) §10.8).
