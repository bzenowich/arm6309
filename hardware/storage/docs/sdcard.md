...
**The logic is built and checked.** `hardware/storage/logic/sdbus.jedec.ts` and
...
against Atmel's CUPL — and `storage/sim/storage_card.v` is these eight packages as a
...
> `hardware/storage/logic/census.ts` counts the pins every net needs and searches every
...
> in full — `hardware/net/docs/net.md` §3.2 cites them — and **§4.4 is the answer, on the read path
...
- **A memory-mapped block buffer fixes it completely** — reads become non-side-effecting
  ...
  more `GAL22V10`s on top of six discretes (`storage/logic/census.ts`, §8.1). §11.1 records
...
plus a latch, so half the eight-byte reservation went spare; `hardware/net/docs/net.md` §5.1 holds
...
`hardware/tools/place/parts.ts` asserts the 8, and `storage/sim/storage_card.v` is these eight
as a board.
...
`hardware/storage/logic/census.ts` lists every net the card's logic needs, records which
...
fuse maps `hardware/tools/gal/jedec/` reads back and executes, so the CPLD's whole case —
...
that, and then a second one inside the kernel — `software/desk/docs/boot-and-desktop.md` §2.
...
⚠ **The host emulator enters at `$8004` by default** — `software/emu/machine.c`, with
boot.asm's handoff already applied — so §10b runs there after a `reboot`, or from reset
with **`COLDBOOT=1`**, which is what `software/desk/video/run-desk.sh` passes to show
...
[`drivewire.md`](../../../software/nitros9/docs/drivewire.md) §3 plans against.
...
[`drivewire.md`](../../../software/nitros9/docs/drivewire.md), which is the owning document.
...
> ⭐⭐ **DECIDED 2026-09-21, IN FAVOUR — `hardware/cpu/docs/plan.md` §4.3.1.** The core
...
| # | Step | Exit criterion |
|---|---|---|
| 0 | **Have a NitrOS-9 to develop the driver under** — boot it from `machine.md` §7.2's ROM disk, and bring up [`drivewire.md`](../../../software/nitros9/docs/drivewire.md) for a *writable* volume. Neither needs this card | NitrOS-9 boots, and a `.dsk` on the host mounts as `/x0` |
...
| 3 | **Program the two GALs** and switch to the fast clock. ⭐ The logic itself is done: `storage/logic/storage.check.ts` is 24 claims against both term lists, including Atmel's CUPL as an independent compiler | burst measured at ≤636 ns on real silicon; the utilisation confirmed on the programmer (**`sdbus` 8 of 10 macrocells, `sdeng` 10 of 10**); **§6.6's `DI` hold margin scoped at the card's own pin**, and the inverted-`SCK` decision taken |
...
12. **Closed** — whether to take the `ATF1508AS` (§8.1), **CLOSED 2026-09-20 by the card
    ...
    that made the *buffered* card affordable at 8 — `storage/logic/census.ts` counts pins
    ...
    `hardware/tools/gal/jedec/` gives a `GAL22V10`. Re-open only if the buffer does.

13. **⭐ The logic is built; the card is not.** What exists and is checked:
    `storage/logic/sdbus.jedec.ts` and `sdeng.jedec.ts`, `storage/logic/storage.check.ts`
    ...
    `storage/logic/census.ts` (the partition search §8.1 rests on, part of
    `make -C hardware check`), the generated `storage/sim/sdbus.v` and `sdeng.v`, and the
    hand-written board model `storage/sim/storage_card.v`. ⚠ **What that does not
...
| [`hardware/net/docs/net.md`](../../net/docs/net.md) | §3.2 cites §4's hazard statement and §4.2's idempotence argument; §5.1 holds `$FF5C`–`$FF5F` |
| **`hardware/storage/logic/`** | ⭐ **the design outputs, which beat this prose where they disagree**: `sdbus.jedec.ts` and `sdeng.jedec.ts` (§8), `storage.check.ts` (24 claims), and `census.ts` (§8.1's partition search). `hardware/storage/sim/storage_card.v` is the board, and `hardware/tools/place/parts.ts` asserts §8's eight |
...
| [`drivewire.md`](../../../software/nitros9/docs/drivewire.md) | the owning document for §11.3 — throughput, the client, the wall clock, and why the boot ROM changed its job |
| ⭐ **`reference/manuals/The 6309 Book (Burke & Burke).pdf`, Appendix 3-139 and Appendix A** | §4.1's `TFM` behaviour, stated outright, **and it IS in `reference/`**; plus `6+3W` cycles in *both* modes, so §5's throughput figures do not improve in native mode. ⚠ The PDF is `.gitignore`d (not redistributable); the opcode and cycle table derived from it is checked in at `hardware/cpu/sim/test/hd6309.tab` |
| ⭐ **NitrOS-9's own sources** — `rb1773.asm`, `llscsi.asm`, `llcocosdc.asm`, `archive/drivers/tccc/tccchd.asm` | §4's hazard, as **practice on real silicon**: every port-source `TFM` masked, one driver abandoning `TFM` outright over it. `hardware/cpu/docs/6309.md` §5.1.5 has the census |
