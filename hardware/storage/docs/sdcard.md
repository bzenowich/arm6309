...
| ⭐ **`reference/manuals/The 6309 Book (Burke & Burke).pdf`, Appendix 3-139 and Appendix A** | §4.1's `TFM` behaviour, stated outright, **and it IS in `reference/`**; plus `6+3W` cycles in *both* modes, so §5's throughput figures do not improve in native mode. ⚠ The PDF is `.gitignore`d (not redistributable); the opcode and cycle table derived from it is checked in at `software/emu/test/hd6309.tab` |
