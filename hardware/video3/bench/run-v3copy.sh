#!/bin/sh
# video3's the copy engine on the host emulator, judged pixel for pixel.
#
#   sh hardware/video3/bench/run-v3copy.sh [seconds]        OUT=dir overrides
#
...
cd "$(dirname "$0")/../../.."
ROOT=$(pwd)
SECS=${1:-4}   # the fill alone is ~1.2 s of machine time
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3copy}
mkdir -p "$B" "$OUT"
sh software/tools/fetch-a09.sh > /dev/null
A09="${A09_DIR:-$(cd "$_here/../../.." && pwd)/.tools/a09}/a09"
python3 hardware/video3/bench/mkv3copy.py hardware/video3/bench/v3copy.asm hardware/video3/bench/v3copy.json > /dev/null
"$A09" -B$B/v3copy.bin -L$B/v3copy.lst hardware/video3/bench/v3copy.asm > $B/v3copy.log 2>&1 || { cat $B/v3copy.log; exit 1; }
grep -qi error $B/v3copy.log && { cat $B/v3copy.log; exit 1; }
python3 - "$ROOT" "$B" <<'PY'
import sys
root, b = sys.argv[1], sys.argv[2]
rom = bytearray(b"\xFF" * (1 << 20))
rom[0:8192] = open(root + "/software/boot/boot.bin", "rb").read()
code = open(b + "/v3copy.bin", "rb").read()
rom[8192:8192 + len(code)] = code
open(b + "/v3copy.rom", "wb").write(rom)
PY
...
VIDEO3=1 "$OUT/emu" $B/v3copy.rom "$OUT" "$SECS" > "$OUT/emu.out" 2> "$OUT/emu.log" || true
...
python3 hardware/video3/tools/v3model.py "$OUT/frames.bin" hardware/video3/bench/v3copy.json
