...
#
#   sh hardware/video3/bench/run-v3char.sh [seconds]        OUT=dir overrides
#
...
ROOT=$(pwd)
SECS=${1:-8}
OUT=${OUT:-$(cd "$_here/.." && pwd)/build/v3char}
mkdir -p "$B" "$OUT"
sh software/tools/fetch-a09.sh > /dev/null
A09="${A09_DIR:-$(cd "$_here/../../.." && pwd)/.tools/a09}/a09"
python3 hardware/video3/bench/mkv3char.py hardware/video3/bench/v3char.asm hardware/video3/bench/v3char.json > /dev/null
"$A09" -B$B/v3char.bin -L$B/v3char.lst hardware/video3/bench/v3char.asm > $B/v3char.log 2>&1 || { cat $B/v3char.log; exit 1; }
...
python3 hardware/video3/tools/v3model.py "$OUT/frames.bin" hardware/video3/bench/v3char.json
