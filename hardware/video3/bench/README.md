# `hardware/video3/bench/` — the host-emulator model

**video3 is modelled in `software/emu/machine.c` beside `video/`, not instead
...
```sh
sh hardware/video3/bench/run-v3.sh            # all four exercisers, ~1 min. Its exit code is the answer
sh hardware/video3/bench/run-v3copyn.sh       # SS.CopyN lands the right pixels
```

⭐ **The application benches that used to live here moved with their programs on
2026-09-23** — each is in its program's own `bench/`, with its record in that
program's README: [`desk`](../../../software/desk/), [`pcs`](../../../software/pcs/),
[`mvania`](../../../software/mvania/), [`monster`](../../../software/monster/),
[`stardew`](../../../software/stardew/), [`tilescroll`](../../../software/tilescroll/),
the retired [`zelda`](../../../software/archive/zelda/), the toolbox's text timing in
[`software/toolbox/bench/`](../../../software/toolbox/bench/) and the SD-card run in
[`software/nitros9/bench/`](../../../software/nitros9/bench/). The retired pinball
table's is in [`software/archive/pinball/`](../../../software/archive/pinball/).
...
`v3model.py` reuses `software/tools/frames.py` so the recording's format is not
...
| ⛔ **Does not answer** | **the hardware's** timing. `make -C hardware/video3 timing` has the arithmetic; the board has the rest |
