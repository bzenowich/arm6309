# `hardware/video3/logic/` — video3's programmable parts

```sh
cd hardware
bun run video3/logic/v3scan.cpld.ts > video3/logic/v3scan.pld       # the term list -> CUPL
sh tools/gal/prjbureau/fit1508.sh video3/logic/v3scan.pld               # place and route
V3_MAPQ=discrete bun run video3/logic/v3scan.cpld.ts > video3/logic/v3scan_mq.pld
sh tools/gal/prjbureau/fit1508.sh video3/logic/v3scan_mq.pld
```
...
is in `archive/video/logic/video.cpld.ts`: every side of both is fitted, and the differences *are* the
