...
`software/monster/bench/run-v3mon.sh` runs it; `bench/README.md` has the whole account. A
...
`software/monster/bench/checkv3mon.py` joins `marks.txt` to `frames.bin` on the
...
`software/archive/pinball/run-v3pin.sh` runs it and `bench/README.md` has the whole
...
The machine has storage (`hardware/storage/docs/sdcard.md` §9.4) and the demos and
their data are on a card, so the constraint §10.1 worked inside is gone.
`software/archive/pinball/mkpcb.py` draws the playfield as a **picture** — a 1984 circuit
