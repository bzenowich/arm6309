...
> draft of this document broke that rule.** `hardware/<card>/logic/cpld/*.fit` describes three
...
>
> ⚠ **`make -C hardware docs` will not catch this.** It compares a utilisation figure
...
`H1`–`H5` cost in `software/nitros9/docs/nitros9-hardware-improvements.md` is a `VDATA` stream that
...
`'165`. `hardware/video3/bench/run-v3sprite.sh` renders every X phase, both 4-byte phases, the
...
2. ⭐ **SETTLED 2026-09-16: `HSCROLL` steps ONE PIXEL, and the second fetch rank stays.**
   ...
   **What the second rank buys is not resolution, it is a floor on speed.** 4 px a frame
   ...
   ⚠ **And the demo does not exercise it.** `software/archive/zelda/bench/mkgame.py`'s overworld
...
3. ⛔ **SETTLED 2026-09-16: the `'153` mux stays, and this trade yields no board room.**
   `make -C hardware/video3 timing` has the arithmetic. The `'153` path — `MUXSEL` → `'153` →
...
4. ⭐ **The partition is [`partition.md`](partition.md): four `ATF1508AS` and one
   ...
   utilisation is recorded in `hardware/<card>/logic/cpld/*.fit`; **those figures describe a
...
15. ⭐ **CLOSED 2026-09-18 — three pin senses, by `make -C hardware pins`.**
...
it**: the host emulator (`software/emu/machine.c`) models `video/`'s card
...
running `software/boot/v3boot/v3boot.asm`. **48 claims, and the first run found one:**
...
wrapper in `hardware/tools/sim/` is.
