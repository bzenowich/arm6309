...
scroll. The video is `software/archive/nitros9-video/video3-demo.mp4`; §8 has what works and
what does not.
...
the old `software/archive/demo` show out of **real NitrOS-9 commands** — desktop, two-pane
...
GrfDrv. It is to **retarget that stack from `video/` to `hardware/video3/`**, and to add the
two things video3 has that `video/` does not: the copy engine and the sprite.

## 1. How far apart the two cards actually are

`defs/armvid.d` holds `video/`'s register map and `hardware/video3/docs/plan.md` §10 holds
...
The brief asked for the old `software/archive/demo` show rebuilt out of **real NitrOS-9
...
`software/paint/tools/v3show.py` as escape streams, which is how a CoCo 3 GUI
...
The camera path is **pre-recorded** by `software/archive/zelda/bench/mkgame.py`, not computed
...
The four term lists now generate Verilog through `tools/sim/emit.ts` — the *same*
...
**`software/archive/nitros9-video/video3-demo.mp4`** — `sh software/archive/nitros9-video/run-video3.sh`
...
`software/desk/video/checkmp4.py` now decides the question instead of a size
...
| 65–127 | its data (`software/toolbox/tools/mktbox.py`): the Haiku palette, **Noto Sans 12 px regular and bold, anti-aliased to two bits**, 31 icons (the old demo's art, re-quantised), and the 384 × 480 paint document |
...
The document is **the photograph `software/archive/demo`'s desktop show was made from**
(`software/toolbox/reference/parrots-image.jpg`, untracked, per git `790de82`), scaled to 480 rows
...
`SHEET=1 sh software/archive/nitros9-video/run-video3.sh` runs the session and writes
...
`software/desk/video/sheet3.py` — about three minutes, where the H.264 file adds several
...
`run-emu.sh`: 23 claims, 0 failed; `SCENARIOS=nitros9 make -C hardware machine`: 19 claims,
0 failed.
...
vertical one, and the hero jumped from tile to tile where `software/archive/demo`'s walks.
...
`software/archive/demo`'s hero is not rebuilt every frame either — on the host emulator it is
...
Asked for after the pointer landed: `software/archive/demo`'s arrow, which is 16 × 16 with a
...
that agree — `vidptr3.asm`'s 64 bytes, `hardware/video3/bench/mkv3sprite.py`'s generator, and the
emulator.
...
`hardware/video3/bench/run-v3sprite.sh` is the check that matters: 36 positions — every X phase,
...
| the art | unchanged since `b446902`, the commit that first ran the demo on the whole machine — the same black is in `software/archive/demo`'s own video |
...
`software/archive/demo`'s emulator run and `checkdemo.py` re-ran against the regenerated model —
...
⭐ **What found it in one run: `WATCH=addr` in `software/emu/machine.c`**, added
...
`software/emu/machine.c` charged a copy like this:
...
`software/toolbox/bench/run-v3text.sh`.

## 16. ⭐ What a toolbox text call costs, measured — 2026-09-18

The first measurement of `tbox.asm` in this repository. `software/toolbox/bench/run-v3text.sh`
...
like before the split. `hardware/video3/docs/keyed-copy.md` carries the rest.
...
`software/archive/nitros9-video/mklegs.py` emits `MaxStep equ <the longest delta>` immediately
...
The brief was to make the NitrOS-9 Paint scene do what `software/archive/demo`'s
...
⭐ The circle comes from `software/archive/nitros9-video/mklegs.py circle 25 56 emit`, the same
