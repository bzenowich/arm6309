...
Superseded claims from [`plan.md`](plan.md), [`signals.md`](signals.md), [`stardew.md`](../../../software/stardew/docs/stardew.md),
[`partition.md`](partition.md), [`keyed-copy.md`](keyed-copy.md),
[`demo-report.md`](demo-report.md) and the two READMEs (`hardware/video3/README.md`,
`hardware/video3/logic/README.md`), with the date each moved and what replaced it.
...
since the reorganisation put a history beside every component's spec.

## `optimizations.md` §10.1 and `bench/README.md` — the pinball table was 61 interned blocks (2026-09-21)

The table became a **file** on 2026-09-21 — `software/archive/pinball/mkpcb.py`'s
...
(`hardware/storage/docs/sdcard.md` §9.4) between the two dates. What the sentence
...
the art in `software/stardew/bench/mkstardew.py`, the bench in `software/stardew/bench/run-v3star.sh`
...
## `hardware/video3/bench/README.md` — "the ROM disk is full" (2026-09-20)
...
`software/nitros9/bench/run-v3sd.sh`); the ROM disk became a rescue system with ~65 K free,
...
> `pinball` was written; its table is 35 K. `software/archive/pinball/run-v3pin.sh` passes
...
bottom row put a fill pixel outside the outline. `software/archive/demo`'s pointer is 16×16, so
...
### `plan.md` header, and `hardware/video3/README.md`'s opening
...
> inherited from [`hardware/archive/video/docs/graphics.md`](../../archive/video/docs/graphics.md) with its
...
bench). `hardware/video3/README.md`'s requirement row read *"**One 8×8 sprite** | the mouse
...
> a tail does not fit in eight rows — `software/archive/demo`'s pointer is 16×16 and its shape is
...
> item 14. `hardware/video3/bench/run-v3sprite.sh` renders every X phase, both 4-byte phases, the
...
>     **+1 IC**, and `hardware/tools/place/parts.ts` does not have it yet.
...
>    `hardware/<card>/logic/cpld/*.fit`; **those figures describe a different design** — one with
...
> `hardware/video3/sim/video3_card.v` is the four parts wired to the discrete parts of
...
> (`hardware/tools/place/parts.ts`, the `video3` alternate), not from a fitter. What this
...
### `hardware/video3/logic/README.md`
