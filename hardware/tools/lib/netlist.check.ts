 ...
 *   bun run tools/lib/netlist.check.ts                                 (motherboard)
 *   bun run tools/lib/netlist.check.ts archive/video/build/board/circuit.json   (video card)
 *
 ...
 */
const path = process.argv[2] ?? "mainboard/build/board/circuit.json"
...
if (board === "video") {
  const J = "J1"                      // the card edge (tools/lib/Card.tsx)
  ...
  console.log(`      ${drawnIcs} ICs drawn of the card's 33 (tools/place/parts.ts) - graphics.md 19 item 34 is open`)
...
for (const net of ["RAM_CE", "RAM_OE", "RAM_WE"]) {
  check(pinsOn("U6", net).length === 0,
    `U6 no longer drives ${net} - the macrocell went to boot mode (mainboard/logic/clkdec.pld)`)
}
...
check(pinsOn("U10", "A23").length === 1 && pinsOn("U10", "A22").length === 1,
  ...
  "not four (mainboard/logic/u9.pld)")
...
check(pinsOn("U10", "nWAIT").length === 0,
  ...
  "controller never stalls the CPU (mainboard/logic/u10.pld)")

/* -- mainboard/logic/u10.pld: the row/column mux select is E, and not a GAL output --- */
...
for (const n of [0, 1]) {
  ...
  check(pinsOn(`U1${4 + n}`, "GND").includes("nOE"),
    ...
    `to ROM space selects nothing instead of fighting the CPU (mainboard/logic/u9.pld)`)
}
{
 ...
 * asserted. This is the same move tools/lib/decode.check.ts makes after the audio
...
/* -- mainboard/logic/mmu.pld: the U3 rewire, 2026-09-06 ------------------------------ */
...
for (const la of ["LA5", "LA4", "LA0"]) {
  check(pinsOn("U6", la).length === 1,
    `U6 takes ${la} - the $FFB1 strobe that leaves boot mode (mainboard/logic/clkdec.pld)`)
}
...
/* -- tools/lib/parts.ts: every pinout is read off a datasheet -------------------
 *
...
check(
  ...
  `tools/lib/parts.ts pinouts: ${stillUnverified.length} unverified, and they are the ${KNOWN_UNVERIFIED.length} expected`,
