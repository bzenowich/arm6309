 ...
 */
import { WINDOWS, MMU_WINDOWS, GEOGRAPHIC_WINDOW } from "./windows"
...
/* each card's drawing lives in its own hardware/<card>/board/, and an
 * archived card's under hardware/archive/<card>/board/ (run from hardware/) */
const boardDirs = [
  ...readdirSync(".").map((d) => `${d}/board`),
  ...readdirSync("archive").map((d) => `archive/${d}/board`),
].filter((d) => d !== "mainboard/board" && existsSync(d))
const boardPath = new Map<string, string>()
for (const d of boardDirs)
  for (const f of readdirSync(d).filter((n) => n.endsWith(".circuit.tsx"))) boardPath.set(f, `${d}/${f}`)
const cardFiles = [...boardPath.keys()].sort()
check(cardFiles.length === 5, "five card boards exist", cardFiles.join(" "))
for (const f of cardFiles) {
  const src = await Bun.file(boardPath.get(f)!).text()
  check(
    /from "(\.\.\/)+tools\/lib\/Card"/.test(src),
    `${f} takes its edge from tools/lib/slot.ts`,
  )
  /* Since 2026-09-08 a card also declares how long it is. tools/place/parts.ts
...
for (const f of cardFiles) {
   ...
   * `video` moved to tools/place/parts.ts's ALTERNATES the day it was archived and
   * `video3` took $FF60 (tools/lib/windows.ts, docs/history.md). The drawing
  ...
  if (name in ALTERNATES) {
    check(!windowCards.has(name),
      `${name} is an alternate (tools/place/parts.ts) and owns no window in the $FF map`)
    continue
  }
