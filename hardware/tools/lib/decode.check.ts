 ...
 * in machine.md, in tools/lib/windows.ts, and in tools/lib/cards.check.ts's header. The
 ...
 * card will break too.
 *
 * tools/lib/cards.check.ts already proves the WINDOWS do not overlap. This proves
...
import { WINDOWS, GEOGRAPHIC_WINDOW } from "./windows"
...
const load = async () => {
  try {
    const { audioCpld } = await import("../../audio/logic/audio.cpld")
    cards.push({ window: "audio", cells: (audioCpld as Merged).cells, note: "audio/logic/audio.cpld.ts" })
  ...
  try {
    ...
    const { v3host } = await import("../../video3/logic/v3host.cpld")
    cards.push({ window: "video3", cells: (v3host as Merged).cells, note: "video3/logic/v3host.cpld.ts" })
...
for (const card of cards) {
  ...
  if (!w) { check(false, `${card.window} has a window in tools/lib/windows.ts`); continue }
   ...
   * product (archive/video/logic/rfa.pld) and never names a cell for it - which is correct and
