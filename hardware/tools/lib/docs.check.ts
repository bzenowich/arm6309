 ...
 * figures that no longer matched `<card>/logic/cpld/*.fit` - and several of them had
...
/* every card keeps its fitter reports in its own logic/cpld/ */
const FITDIRS = ["audio", "video3", "archive/video"].map((c) => join(ROOT, "hardware", c, "logic", "cpld"))
...
for (const FITS of FITDIRS) for (const f of readdirSync(FITS).filter((n) => n.endsWith(".fit"))) {
...
const EXEMPT = (p: string) =>
  ...
  /* ⭐ hardware/archive/ - 2026-09-20. An archived card's documents are a FROZEN RECORD
   ...
   * a checker quiet. `hardware/archive/video/docs/graphics.md` states vaddr, vctrl and
   * vsup's utilisation as it stood when the card was retired; those .fit files
   * are still in <card>/logic/cpld and still agree, and the day they stop being
   * regenerated the prose must not start failing. See hardware/archive/README.md. */
  /^(hardware|software)\/archive\//.test(p) ||
  /(^|\/)node_modules\//.test(p) ||
  /(^|\/)\.agents\//.test(p) ||
  /(^|\/)reference\//.test(p) ||
  /(^|\/)coco3_c64\.md$/.test(p) ||
 ...
 * its unit, then the first in the unit. In hardware/audio/ the parts are also called U1
...
const ALIASES: [RegExp, RegExp, string][] = [
  [/^hardware\/audio\//, /\bU1\b/g, "audio"],
  [/^hardware\/audio\//, /\bU2\b/g, "aseq"],
]
...
check(wrong.length === 0,
  "every present-tense utilisation figure in the specs matches <card>/logic/cpld/*.fit - " +
  "the fitter is the authority and prose does not get a second opinion",
  wrong.length ? `${wrong.length} stale` : "")

/* -- IC counts, against tools/place/parts.ts ------------------------------------
 *
 ...
 * it. `tools/place/parts.ts` is the authority because `place.check.ts` already
 ...
 * card, hardware/audio/** is the audio card. Shared documents are checked only where
...
{
  /* ⭐ `hardware/video3/` SINCE 2026-09-20, and `video/` left the same day - the card
  ...
  const OWNER: Record<string, string> = {
    "hardware/video3/": "video3", "hardware/audio/": "audio",
    "hardware/net/": "net", "hardware/storage/": "storage", "hardware/io/": "io",
  }
  ...
  if (badIc.length) {
    ...
    for (const c of badIc) {
      console.error(`      ${c.file}:${c.line}  ${c.card}: says ${c.value} ICs, ` +
        `tools/place/parts.ts says ${CARDS[c.card].ics}`)
      console.error(`        ${c.text.slice(0, 130)}`)
    }
    console.error("")
  }
  check(badIc.length === 0,
    `every present-tense IC total in the specs matches tools/place/parts.ts ` +
