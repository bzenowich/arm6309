 ...
 * hardware/archive/video/docs/graphics.md 14 and hardware/audio/docs/audio.md 12 both already assume".
 * Drawing the boards is what settled it: tools/place/place.check.ts shows the video
 ...
 *
 * Every card takes /RESET as an input. hardware/io/ps2/docs/ps2.md did not until the
...
export const Card = ({ name, length, children }: CardProps) => (
  <board name={name} width={`${length}mm`} height={`${CARD_HEIGHT_MM}mm`} routingDisabled>
    {/* J1 is the edge, always. Its pinout is tools/lib/slot.ts and nothing else. */}
