# `hardware/tools/gal/jedec/` — the fitter, and what it is worth
...
The designs are [`../mmu.jedec.ts`](../../../mainboard/logic/mmu.jedec.ts) and
[`../clkdec.jedec.ts`](../../../mainboard/logic/clkdec.jedec.ts); the check is
[`../jedec.check.ts`](../jedec.check.ts), and `make -C hardware jedec` runs it.
...
1. **`hardware/tools/gal/reference/ATF22V10C.pdf`.** §10's compiler-mode table gives
...
and `make -C hardware cupl` executes them on our fuse map. Both errors were exactly
