 ...
 *
 * tools/gal/README.md open item 1 says "nothing has been fitted" and that the pin
...
import { RESET_STATE, decode, setsRun, step, type Counter } from "../../mainboard/logic/clkdec.model"
import { MAP, paOf, u9 } from "../../mainboard/logic/u9.model"
import { BANK, RESET_STATE as U10_RESET, accessWindow, casWindow, out as u10out, refreshSafe, step as u10step, type State as U10State } from "../../mainboard/logic/u10.model"
...
/* the four GALs this check builds are the motherboard's */
const OUT = join(here, "..", "..", "mainboard", "logic")
...
const build = (design: Design, base: string) => {
  ...
  writeFileSync(join(OUT, `${base}.jed`), jed)
  writeFileSync(join(OUT, `${base}.doc`), toReport(design, a))
...
const u3 = build((await import("../../mainboard/logic/mmu.jedec")).mmuDesign, "mmu")
 ...
 * needs an enable of its own, so the part takes tools/gal/README.md's other budget
...
const u6 = build((await import("../../mainboard/logic/clkdec.jedec")).clkdecDesign, "clkdec")
 ...
 * the same function - the step tools/gal/README.md counts product terms for and
 ...
 * everywhere else, so the rule is worth a check rather than a memory. */
{
  ...
  /* every card's logic/ directory, and the archive's */
  const HW = join(here, "..", "..")
  const dirs = ["mainboard", "audio", "video3", "storage", "archive/video"].map((c) => join(HW, c, "logic"))
  const bad: string[] = []
  for (const dir of dirs) for (const f of readdirSync(dir).filter((n) => n.endsWith(".pld"))) {
...
const u9g = build((await import("../../mainboard/logic/u9.jedec")).u9Design, "u9")
...
const u10g = build((await import("../../mainboard/logic/u10.jedec")).u10Design, "u10")
