...
# the design's own logic/cupl/, which is what jedec/cupl.check.ts validates against.
#
...
#
# Usage: cupl-reference.sh mainboard/logic/mmu.pld [device]     (from hardware/)     (device defaults to g22v10)
...
# the reference lands beside its design: <card>/logic/foo.pld -> <card>/logic/cupl/
REF=$(cd "$(dirname "$PLD")" && pwd)/cupl
