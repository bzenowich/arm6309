# `hardware/tools/gal/` — the programmable logic, and how it is checked

**`hardware/archive/video/docs/graphics.md` §18 step 2 is "fit the GALs", and §18 says
...
> card's Verilog from them and `demo_tb` still runs `software/archive/demo/`'s show against the
...
> `make -C hardware check` and five testbenches in `verilog/run.sh`'s default `TBS`. Everything
...
decode) and `U10` (the SIMM controller, [`../ram.md`](../../mainboard/docs/ram.md) §6.3). `mmu.jed`,
...
**Still unwritten:** **the I/O card's pair** ([`../cards/io.circuit.tsx`](../../io/board/io.circuit.tsx)), which
...
| | |
|---|---|
| [`mmu.pld`](../../mainboard/logic/mmu.pld) | **U3, the MMU sequencer** — CUPL, the deliverable a fitter consumes |
| [`mmu.v`](../../mainboard/sim/mmu.v) | the same seven equations in Verilog |
| [`mmu_tb.sv`](../../mainboard/sim/mmu_tb.sv) | 16 claims against `mmu.v` under **Verilator** — `make -C hardware sim-hand` |
| [`mmu.model.ts`](../../mainboard/logic/mmu.model.ts) | the same seven equations as arithmetic |
| [`mmu.check.ts`](../../mainboard/logic/mmu.check.ts) | 23 claims against that model — `make -C hardware check` |
| [`clkdec.pld`](../../mainboard/logic/clkdec.pld) | **U6** — the E/Q divider, `/IOSEL`, and **boot mode**: the `RUN` latch and the boot buffer's output enable |
| [`clkdec.v`](../../mainboard/sim/clkdec.v) + [`clkdec_tb.sv`](../../mainboard/sim/clkdec_tb.sv) | the same, and 15 claims under Verilator |
| [`clkdec.model.ts`](../../mainboard/logic/clkdec.model.ts) | U6's behaviour, *not* as a sum of products — the reference the expansion is checked against |
| [`mmu.jedec.ts`](../../mainboard/logic/mmu.jedec.ts), [`clkdec.jedec.ts`](../../mainboard/logic/clkdec.jedec.ts) | the terms, placed in macrocells |
| [`jedec/`](jedec/) | **the fitter** — assembler, fuse-map simulator, and the device description both depend on |
| [`jedec.check.ts`](jedec.check.ts) | assembles both parts and checks **the fuses** against the models — `make -C hardware jedec` |
| [`u9.pld`](../../mainboard/logic/u9.pld) | **U9, added 2026-09-09** — the physical space decode: the boot ROM, the four SIMM windows, the backplane's `/IOPAGE`, and the two map SRAMs' chip enables |
| [`u9.jedec.ts`](../../mainboard/logic/u9.jedec.ts) + [`u9.model.ts`](../../mainboard/logic/u9.model.ts) | the terms placed in macrocells, and the same decode as ranges and comparisons |
...
| | |
|---|---|
| [`sync.timing.ts`](../../archive/video/logic/sync.timing.ts) | the two VGA timings, with the counter-origin choice that makes `VSYNC` one shared product term — and an arithmetic self-check that caught `graphics.md`'s 60.0 Hz |
| [`sync.jedec.ts`](../../archive/video/logic/sync.jedec.ts) | **`hgen`, `vgen`, `vdec`** — the trio, and why it is a trio |
| [`sync.model.ts`](../../archive/video/logic/sync.model.ts) | the raster as arithmetic: counters are numbers, windows are comparisons |
| [`sync.check.ts`](../../archive/video/logic/sync.check.ts) | a whole frame in each family, off the fuses — `npm run check:sync` |
...
| | |
|---|---|
| [`scan.jedec.ts`](../../archive/video/logic/scan.jedec.ts) | **`hadr`, `vadr`** — the column and row counters, and why there is no carry between them |
| [`scan.model.ts`](../../archive/video/logic/scan.model.ts) | the 1024 × 512 torus as arithmetic |
| [`scan.check.ts`](../../archive/video/logic/scan.check.ts) | every address of a 400-line frame at three scroll positions — `npm run check:scan` |
...
| | |
|---|---|
| [`access.jedec.ts`](../../archive/video/logic/access.jedec.ts) | **`arb`, `wcol`, `wrow`** — the spare-access grant logic and the 19-bit pointer |
| [`access.model.ts`](../../archive/video/logic/access.model.ts) | §5.2.1's grant rule verbatim, and the pointer as arithmetic |
| [`access.check.ts`](../../archive/video/logic/access.check.ts) | all 128 arbiter inputs, and the wrap that decides item 12 — `npm run check:access` |
...
| | |
|---|---|
| [`seqph.jedec.ts`](../../archive/video/logic/seqph.jedec.ts) | **`seqph`** — dot phase, slot tick, §5.2.2's sub-slot split, four per-chip fetch-latch clocks, pixel mux select |
| [`seqph.check.ts`](../../archive/video/logic/seqph.check.ts) | the phase, that the slot tick does **not** move with `HSCROLL`, and the arithmetic that shows a common fetch-latch clock cannot render a scrolled line — `npm run check:seqph` |
| [`seqctl.jedec.ts`](../../archive/video/logic/seqctl.jedec.ts) | **`seqctl`** — the span writer, respecified for 8 × 8 cells (`graphics.md` §7.4) |
| [`seqctl.model.ts`](../../archive/video/logic/seqctl.model.ts) | its state machine: one handshake, three terminations |
| [`seqctl.check.ts`](../../archive/video/logic/seqctl.check.ts) | all 16 states × 128 inputs, and the eight-byte glyph row — `npm run check:seqctl` |
| [`tile.model.ts`](../../archive/video/logic/tile.model.ts) | §6.4's three address concatenations — bitmap, 8bpp tile, 1bpp character |
| [`cadence.check.ts`](../../archive/video/logic/cadence.check.ts) | §6.4.9's cell-mode fetch sequence over **a whole 800-dot line** — 80 map accesses for 80 cells in order, the one-cell lead measured in dots, §6.4.2's 9-per-8-dots and 2.25-per-chip, one source on the address bus, the four `/WAIT` cases — **and §8.1's window signals over a whole frame in each of the four `VMODE` codes**: which row each displayed line shows at five scroll positions, §6.2's line doubling, and every counter enable counted **per dot**, which is what catches a missing `SLOTTICK` gate — `npm run check:cadence` |
| [`tile.check.ts`](../../archive/video/logic/tile.check.ts) | the no-adder property, asserted as OR = ADD over all 524,288 field combinations per variant — **and that the fitted `addressMux()` computes the modelled tile and map addresses**, over every cell, every pixel within it and all 256 codes — `npm run check:tile` |
...
write-decode GAL"*; the equations implement it, `mainboard/board/mainboard.circuit.tsx` wires
...
> byte and a wider write path — which is what [`../ram.md`](../../mainboard/docs/ram.md) §3.1 specifies.
...
| **6 — both off the part** | **16** | **15** | ✓ one pin spare, and it is spent below |
| ⚠ **7 — with `ISOOE_HI`** | **15** | **15** | ⚠ **the row it takes now: nothing spare, U3 is FULL** |
...
**So it fits — and it did not fit as drawn.** That is `graphics.md` §18 step 2's exit
criterion answered for this GAL, and answered "yes, but only after three changes."

⚠ **AND THE SPARE PIN IS SPENT: U3 IS FULL, 7 outputs and 15 inputs of 15.** It went on
`ISOOE_HI`, the high map SRAM's isolation enable, when `design-review2.md` M-1's repair
turned out to be half a repair — the board never wired `D0`–`D7` to the high map SRAM at
all, and two common-I/O SRAMs cannot share one `'245` because each drives its own `DQ`
...
> **56 of 64 I/O and 95 of 128 cells** since 2026-09-13, placed on the fitter's
...
[`../../reference/datasheets/`](../../../reference/datasheets/) and from `hardware/cpu/docs/plan.md`
...
`PIN 23` was then left undeclared, so the macrocell held it at high-Z and it was a
sixteenth input if the board ever wanted one.

⚠ **AND THE BOARD WANTED ONE.** Pin 23 is `ISOOE_HI` since 2026-09-09 — a real output
with a real net, not a driven zero — so U3 takes the budget table's *other* row and has
nothing left. `jedec.check.ts` asserts the current state: *pin 23 is driven —
`ISOOE_HI`.* The superseded reasoning, and `ram.md` §11 item 5's closure on it, are in
[`../history.md`](../../history.md). The lesson survives the reversal intact: **a driven
pin is an output, whatever it is driving**, and the table and the file must agree.
...
(`WAIT.oe = SPANBUSY & VRAMSEL & !IOPAGE & E & !RW`, [`access.jedec.ts`](../../archive/video/logic/access.jedec.ts))
...
be reset — and `tools/lib/netlist.check.ts` asserts that U6 takes the signal at all, which is
...
| `vctrl` | **56 of 64** | **95 of 128** | sync, sequencer, span control, the arbiter, `CTRL` |
| `vsup` | **63 of 64** | **94 of 128** | register-file address, `SPANLEN`, §8.2's rank select, §9's palette write path, §10.3's descriptor decode |
...
argument. The arbiter is still merged into `vctrl`, where `WRITESEL` **is** `SPNGRANT`.

⛔ **`vaddr` is bound by LAB FAN-IN, which is neither of the columns above.** An
`ATF1508AS` block sees 40 signals through the switch matrix and `vaddr` sits at **40 of
40 in all eight blocks**. On 2026-09-09 three separate *one-literal* changes to the
display list were refused there — two `## Warning : Grouping fail / Design does not
fit`, one `INTERNAL ERROR` — and each left the previous `.fit` in place, so only the md5
...
them costs logic pins: `vaddr` 59 + 4 = 63 pins and `vctrl` 53 + 4 = 57 have room, and
`vsup` 61 + 4 = 65 does not. `vctrl` has 35 cells spare since `graphics.md` §19 item 48's
refit put it back on the fitter's first pass.
...
| Digital verification | **Verilator** 5.020 | ✓ `make -C hardware sim-hand`, 16 claims, `-Wall` clean |
| Analogue — the video output stage (`design-review.md` §Vid-M4) and the audio ladder | **ngspice** | ✓ installed, **nothing written yet** |
| Equations | `bun`, already here | ✓ `make -C hardware check`, 23 claims |
| A CPU to drive it | [`../vendor/mc6809`](../../cpu/sim/mc6809) — Greg Miller's cycle-accurate MC6809E, BSD | ✓ elaborates; **nothing drives it yet** |

Four GALs ship, all of them the motherboard's, out of the roughly
twenty programmable parts the machine carries across its boards. `make -C hardware sim-hand` runs
the two motherboard testbenches; `make -C hardware check` runs the equation check.
...
`tools/gal/minimise.probe.ts` runs the first past CUPL's own minimiser;
`tools/gal/polarity.probe.ts` runs the second past `jedec/twolevel.ts`, a
...
3. **⚠ Three decodes are unwritten, and two of them grew on 2026-09-09.**
   ...
   - **the I/O card's pair** ([`../cards/io.circuit.tsx`](../../io/board/io.circuit.tsx)).
...
behind it — which is how `archive/video/logic/vctrl.pld` and `video.cpld.ts` drifted apart.
...
`make -C hardware pld` writes the three CPLD sources; `make -C hardware sim`
regenerates the Verilog and runs six testbenches over whole frames, whole
spans and the boot sequence. `make -C hardware all` does the lot.
