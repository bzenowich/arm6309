/* Renders the placement study to tools/build/boards.html.
 *
 * tools/place/page.html is the prose with three placeholders; everything numeric in
 * it comes from parts.ts through pack.ts, so a card that grows a package
 * redraws rather than going stale. The stylesheet is tools/place/style.css and is
...
writeFileSync(join(here, "../build/boards.html"), html)
console.log(`wrote tools/build/boards.html — ${ORDER.length} cards + motherboard, ${html.length} bytes`)
