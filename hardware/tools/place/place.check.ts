 ...
 *
 * <card>/board/*.circuit.tsx carries `icBudget`, transcribed from the card's own
...
for (const f of ["video", "audio", "net", "storage", "io"]) {
  let src: string
  try { src = readFileSync(join(here, "..", "..", f === "video" ? "archive/video" : f, "board", `${f}.circuit.tsx`), "utf8") }
  catch { check(false, `${f}/board/${f}.circuit.tsx exists`); continue }
  const m = src.match(/icBudget=\{(\d+)\}/)
  if (!m) { check(false, `${f}/board/${f}.circuit.tsx declares icBudget`); continue }
 ...
 * that day and `archive/video/board/video.circuit.tsx` is still drawn, so iterating CARDS
...
for (const [key, c] of [...Object.entries(CARDS), ...Object.entries(ALTERNATES)]) {
  ...
  check(icCount(c) === b, `${key}: and agrees with ${key}/board/${key}.circuit.tsx`,
 ...
 * hardware/tools/place/parts.ts. Until each has an entry and a board file with an
 ...
 * ever say so. This is the assertion that keeps tools/place/parts.info.ts complete
