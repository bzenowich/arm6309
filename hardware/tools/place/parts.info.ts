...
export const FAMILY: Record<string, string> = {
  ...
  "74HCT574": "Octal D flip-flop with 3-state outputs and a common clock, TTL-level inputs.",
...
export const ROLE: Record<string, string> = {
  ...
  "video:74HCT244 lbyte": "Puts the display list's fetched descriptor byte onto the card's internal data bus, so a `MOVE` operand reaches the same register write path the CPU uses. §10.3.3.",
  ...
  "video:74HCT245 rdbk": "Bridges the card's internal data bus to the backplane for register read-back — and stands off at `+$13` VSTAT and `+$15` VDATA, whose bytes come from other parts.",
  "video:74HCT574 vread": "The VRAM read latch — §11. Filled from the pixel bus at WPTR by vsup's prefetch before the CPU asks, and driven onto D0–D7 for a read of the VRAM window or of `+$15` VDATA, which then post-increments WPTR.",
  ...
  "audio:74HC574 conv port": "One per channel, three-stated in pairs onto the left and right converter ports; the frame parity S3 says which pair drives. A sample is loaded at its PEND write, a volume code by W5 (§6.2).",
  "audio:74HC138 conv ctl": "Decodes U2's 3-bit code into the four converter port registers' clocks. The DAC chip selects left it on 2026-09-11 for registered U2 pins, because a decoder output glitches when a multi-bit code changes (§6.2).",
  ...
  /* ---------------- storage ---------------- */
  "storage:GAL22V10": "Two of them. sdbus is the $FF58 decode, the four register strobes and SDSTAT's three bits; sdeng is SDCTRL and the engine that turns one bus access into exactly eight SPI clocks. sdcard.md §8, and storage/logic/census.ts for why the split falls where it does.",
...
export const MB_ROLE: Record<string, { fam: string; role: string }> = {
  ...
  "U6 GAL22V10 E/Q": { fam: "Electrically-erasable PLD, 10 macrocells.",
    role: "The clock divider: E and Q in quadrature from the 25.175 MHz master, \u00f712 or \u00f78. Also `/IOSEL`, `/BOOTOE` and the `RUN` latch. Every registered macrocell carries a `/WAIT` hold term, which is how a card stops the machine. `mainboard/logic/clkdec.pld`." },
