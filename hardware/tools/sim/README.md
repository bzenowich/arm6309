...
`make -C hardware machine` is the machine with `video3` in the slot.

## Why this exists

`hardware/tools/gal/`'s checks are exhaustive and they are all the same shape: execute
...
| `video_card.v` | hand-written: the **archived** `video` card wired up — three parts, four interleaved framebuffer chips, the fetch latches and the `'153` mux. Board-level, transcribed from `hardware/<card>/board/` and `video.cpld.ts`. ⚠ Kept because `machine.v` instantiates it and `demo_tb` instantiates that; `boot.asm` no longer drives it |
...
| `mainboard.v` | hand-written: U3, U6, U9, U10, both map SRAMs, the `'157`, the `TASK` `'574`, the boot `'244`, both flash devices and the SIMM bank. Transcribed from `hardware/mainboard/board/mainboard.circuit.tsx`, which is the netlist of record |
| `tl16c550.v` | hand-written, and ⚠ **a bus model of a bought part, not a design output**: the serial card's TL16C550C (`hardware/io/serial/docs/serial.md` §7), for `machine.v`'s and `machine3.v`'s `SERIAL = 1`. It is the console that `machine_tb +scenario=nitros9` types at, and it must behave as `software/emu/machine.c`'s UART does |
...
and are run by `make -C hardware sim-hand`; `mainboard.v` instantiates them rather than
a second copy.
