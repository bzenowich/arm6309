...
// backplane this wires; hardware/video3/docs/plan.md §10 is the card's register map.
//
// ⭐ WHY THIS EXISTS. machine.v does exactly this for video/, and
// hardware/video3/docs/plan.md §15.2 says video3's machine bench "should be that one
...
//               $FF38 - hardware/io/serial/docs/serial.md §7.1 - with its INTR on the
...
//               hardware/storage/docs/sdcard.md §3.1 - with sd_model.v, a behavioural
...
module machine3 #(
  ...
  // from the board. hardware/cpu/sim/mc6809/README.md is why this core.
  wire [7:0] cpu_d_in;
  wire [7:0] cpu_d_out;
  wire [15:0] cpu_addr;
   ...
   * loop in software/boot/v3boot then EXITS EARLY, and the card looks like it
