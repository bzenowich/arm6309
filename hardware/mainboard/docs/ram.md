...
[`mainboard/board/mainboard.circuit.tsx`](../board/mainboard.circuit.tsx), which
...
From [`tools/gal/README.md`](../../tools/gal/README.md), `docs/machine.md` §6.3.1, §7.1 and §7.2, and
§§3, 5, 6 and 6.7 below:
...
`tools/gal/README.md` records it without drawing the conclusion: *"Sixteen 8-bit
...
have. `vctrl` has the pins, at **56 of 64 I/O** (`archive/video/logic/cpld/vctrl.fit`), and it has cells
...
**32 packages**, which is not a motherboard — [`place/`](../../tools/place/) already puts
...
| | |
|---|---|
| Reserved footprints | **removed** — `tools/place/svg.ts` |
...
> **2026-09-09.** [`mainboard/logic/u10.pld`](../logic/u10.pld), [`mainboard/logic/u10.jedec.ts`](../logic/u10.jedec.ts)
> and [`mainboard/logic/u10.model.ts`](../logic/u10.model.ts). `make -C hardware jedec` steps the fuses and
> the model together over **1,920 clock edges** — every bus phase, DRAM cycle and not,
> both `R/W` directions, across many refresh bursts — and `make -C hardware cupl` runs
...
on the bus during a write cycle, which is the contention `tools/gal/README.md` argues about for
the system RAM's `/OE`.
...
`A0` splits `$FFB0` (TASK) from `$FFB1` (the strobe). `mainboard/logic/clkdec.pld`.
...
**not connected** — [`tools/lib/parts.ts`](../../tools/lib/parts.ts)'s `SIMM30` is written from the pinout
...
`mainboard/sim/mainboard_tb.sv` also runs the walk, as literal bus cycles against `mainboard.v`, which
...
⭐ **`mainboard.circuit.tsx` draws all nineteen**, and `make -C hardware netlist` asserts
...
(`archive/video/logic/access.jedec.ts`) — it is asserted only when the CPU is **writing VRAM**; reads
...
> **2026-09-09.** [`mainboard/logic/u9.pld`](../logic/u9.pld), [`mainboard/logic/u9.jedec.ts`](../logic/u9.jedec.ts) and
> [`mainboard/logic/u9.model.ts`](../logic/u9.model.ts); `make -C hardware jedec` evaluates the fuses against
> the model over all 16,384 input combinations and `make -C hardware cupl` runs **Atmel's own
...
[`tools/lib/slot.ts`](../../tools/lib/slot.ts) spent its last position on physical `A20` — and
...
| ⚠ **U3's second isolation enable** | **0 ICs, and U3's last pin** | pin 23. The part is now **7 outputs, 15 inputs, nothing spare** — `tools/gal/README.md`'s other budget row. **Anything else the motherboard's decode ever wants is a new package** |
...
10. **⚠ NEW — nothing burns the ROM yet.** §6.7 specifies the part and the decode; the
    *contents* — boot monitor, the `$FFC0`–`$FFFF` vector table, the ROM-disk image and
    ...
    exist. [`../docs/drivewire.md`](../../../software/nitros9/docs/drivewire.md) §6.1 is one of its callers,
...
| | |
|---|---|
| [`tools/gal/README.md`](../../tools/gal/README.md) | the MMU's register map, the entry format, and the "16 of 2048" line §3.2 turns into a capability |
...
| [`place/`](../../tools/place/) | the placement study — `svg.ts` draws the motherboard at **19 ICs**, four SIMM sockets and the boot ROM, with no SRAM outside the map |
| [`mainboard/board/mainboard.circuit.tsx`](../board/mainboard.circuit.tsx) | the `'574` with seven unused bits — and U3's pin 23 is **spent** since 2026-09-09 (§11 item 11) |
...
| [`../docs/drivewire.md`](../../../software/nitros9/docs/drivewire.md) | what the boot ROM's spare megabyte is *not* for, and how a new one gets onto the machine |
