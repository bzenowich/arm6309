 ...
 * own CUPL - U3 (mainboard/logic/mmu.pld), U6 (mainboard/logic/clkdec.pld), U9 (mainboard/logic/u9.pld) and U10
 * (mainboard/logic/u10.pld). There is no unwritten logic on this board.
 */
import { SlotSocket } from "../../tools/lib/SlotConnector"
import {
  CPU_SOCKET, MAP_SRAM, HCT574, HCT245, HCT157, HCT244, FLASH_512K, SIMM30,
  HC4040, gal22v10, labels,
} from "../../tools/lib/parts"
...
export default () => (
  <board name="arm6309-mainboard" width="220mm" height="180mm" routingDisabled>
    {/* ------------------------------------------------ CPU module socket -- */}
    {/* The module is the 40-pin drop-in board hardware/cpu/ already builds for the
    ...
    <chip
      ...
      connections={{
         ...
         * (mainboard/logic/u9.pld, tools/gal/jedec.check.ts). */
    ...
    {/* U2 - task select. ONE bit of eight, not the three this comment used to
      * claim (tools/gal/README.md findings 1 and 3): MMU_EN was dropped because no
    ...
    {/* U3 - the MMU sequencer. mainboard/logic/mmu.pld is the source of truth for these
      * pins and this board follows it; tools/gal/README.md carries the derivation.
      *
      ...
      * tools/gal/README.md's pin budget had two rows and the board took the first,
    ...
    {/* U4 - break-before-make isolation between U1's common I/O and D0-D7.
      ...
      * E-rise, which puts ~174 ns between the two - tools/gal/README.md.
      *
    ...
    {/* U18 - the HIGH map byte's isolation '245, and it is the second half of
      ...
      * mainboard/logic/mmu.check.ts asserts the two are never asserted together.
      *
    ...
    {/* U5 - the quad 2:1 the mux argument in graphics.md 6.3.1 costs a package
      ...
      * the instant the write strobe asserted (tools/gal/README.md finding 2). It is
    ...
    {/* U6 - the divider and boot mode. 25.175 / 12 = 2.0979 MHz, and that is
      ...
      * t_cyc minimum.
      *
      * mainboard/logic/clkdec.pld is the source of truth for these pins. Three jobs:
      *
    ...
    <chip
      ...
      connections={{
         ...
         * released at 10 (mainboard/logic/u10.pld). They were on pins nothing connected
    ...
    {/* ------------------------------------------- the space decode ------- */}
    {/* U9 - which memory, if any, is this? mainboard/logic/u9.pld is the source of truth
      * and tools/gal/jedec.check.ts checks its fuses against mainboard/logic/u9.model.ts over
      ...
      * hardware/mainboard/docs/ram.md 4.3, docs/design-review2.md M-1. */}
    <chip
      name="U9"
      footprint="dip24_w0.3in"
      pinLabels={labels(gal22v10({
    ...
    {/* machine.md 7.2, 1 MB at physical 2.0-3.0 M. It holds the boot monitor
      ...
      *
      * ⚠ THE PINOUT IS UNVERIFIED - tools/lib/parts.ts FLASH_512K names the two
      ...
      *
      * /OE is tied low and R/W rides in the chip enable (mainboard/logic/u9.pld) - so a
    ...
    {/* ------------------------------------------------ system memory ----- */}
    {/* U10 - the SIMM controller. mainboard/logic/u10.pld is the source of truth and
      * tools/gal/jedec.check.ts checks its fuses against mainboard/logic/u10.model.ts over 1,920
    ...
    {/* U11-U13 - the RAS/CAS address mux, 11 bits. A 4 MB 30-pin SIMM is
      ...
      *
      * ⭐ SEL IS `E`, not a GAL output (mainboard/logic/u10.pld). A '157's pin 1 is A/B
