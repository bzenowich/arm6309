...
| **Control store part** | **`SST39SF040`** (512K×8, 70 ns, PDIP-32, 5 V) — the part the motherboard's boot ROM already uses | One verified pinout ([`hardware/tools/lib/parts.ts`](../../tools/lib/parts.ts) `FLASH_512K`), one programmer, one supply line for the whole machine. ⚠ 0.4 % of it is used — §4.4 |
...
| **IC count** | ⛔ **62 packages as budgeted, and they do not place** — against `graphics.md` §14.1's 33 | Measured with `hardware/tools/place/pack.ts`, not estimated: 7 packages over on the longest board the machine has. **Two levers close it**; §11.2 has the numbers and §15 item 0 is the decision they force |
...
| | |
|---|---|
| **Window** | `$FF60`–`$FF7F`, 32 bytes — `machine.md` §3, `hardware/tools/lib/windows.ts`. **Taken by video, and `video2` is video** |
...
rather than `A18`** (`hardware/tools/lib/parts.ts` `FLASH_512K` records that trap). Either
...
**59.94 Hz, not 60.0** — `hardware/archive/video/logic/sync.timing.ts`'s arithmetic, and it matters for
...
`hardware/archive/video/logic/sync.timing.ts` already carries and `npm run check` already asserts —
§13.
...
`hardware/tools/lib/parts.ts`: the module fronts the bus with 3.3 V buffers, and a `74HC`
...
**`hardware/tools/place/pack.ts` is the repository's own skyline packer**, and the BOM above
...
> `tools/lib/docs.check.ts` does not guard courtyard figures, only utilisation and IC totals.
...
| | |
|---|---|
| **`npm run gen:ucode`** | assembles `video2/ucode/*.ts` into the two ROM images and a listing. ⚠ **Nothing is hand-assembled and no image is checked in** — the same rule `hardware/tools/sim/`'s generated Verilog keeps |
...
| **`check:netlist`** | ⛔ `graphics.md` §19 item 34 is that `archive/video/board/video.circuit.tsx` is a partial board and *"that is where the next M-1 will hide"*. `video2` draws the whole card or it does not claim the check |
...
| # | Step | Exit criterion |
|---|---|---|
| 0 | ⛔ **Decide which two levers close §11.2's seven packages**, then put the BOM in `hardware/tools/place/parts.ts` and a board file in `hardware/<card>/board/` so `check:place` asserts it rather than a scratch script | the card places on 240 mm **from the repository's own parts list**. ⚠ **Before anything else, and it is a design decision and not a counting exercise** — §11.2.1 |
...
8. ⚠ **Nothing here is drawn.** No `hardware/cards/video2.circuit.tsx`, no
   `hardware/tools/place/parts.ts` entry, no ROM images, no testbench. **This document is a
...
- [`hardware/README.md`](../../README.md) — the 72-pin slot, the card format and
  the checks. [`hardware/tools/lib/parts.ts`](../../tools/lib/parts.ts) — `FLASH_512K`, the
