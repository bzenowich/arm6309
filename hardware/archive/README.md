# `hardware/archive/` — retired designs, kept whole
...
`hardware/archive/` describes the design as it stood on the day it was retired, and the date and
the reason are recorded below and in [`../docs/history.md`](../../docs/history.md).

## ⭐ An archived document is still citable

**`hardware/archive/video/docs/graphics.md` is where this machine was designed.** The 72-pin
...
`hardware/audio/docs/audio.md`, `hardware/io/`, `hardware/net/`, `hardware/storage/`, `hardware/tools/lib/slot.ts` and
`hardware/video3/docs/plan.md` all cite it, and **they are right to.**
...
| | Archived | Why |
|---|---|---|
| [`video/`](video/) | **2026-09-20** | The machine's specified video card until that date: 640×200 × 256 colours, 80×25 text, a display list, byte-granular scroll, a span writer — **33 ICs on a 24 cm board, three fitted `ATF1508AS`**, simulated end to end and executed by a 6809E out of its own boot ROM. ⭐ **[`hardware/video3/`](../video3/) replaced it**, which does character mode with per-cell colour, a keyed copy engine and a sprite, and which NitrOS-9 and ANSI art need and `video/` cannot give (`software/nitros9/docs/video-compat.md`). Superseded, not wrong |
| [`video2/`](video2/) | **2026-09-20** | A microcoded ANSI text card — two flash control stores, no CPLDs, no GALs, hardware character generator, fixed RGB332. **Paper only**: a plan and a datapath PDF, never built, never fitted, never placed in `hardware/tools/place/parts.ts`. `video3` took the question it was asked to answer |
...
still `hardware/archive/video/board/video.circuit.tsx`. That is deliberate and it is a compromise:

- `hardware/tools/sim/gen.ts` emits `vaddr.v`, `vctrl.v`, `vsup.v`, `rfa.v`, `vlen.v`
  ...
  holding these files here is `software/archive/demo/`, whose raster bars and per-scanline
...
- `hardware/tools/gal/fold.check.ts` uses the video arbiter as the **fixture** for
  ...
  card.
- `tools/place/parts.ts` keeps the card in `ALTERNATES`: placed, length-checked and totalled
  against its own 33, and tied to `archive/video/board/video.circuit.tsx`'s `icBudget`. An alternate
  owns no `$FF` window, which is what `tools/lib/cards.check.ts` now asserts of it.

⛔ **What that costs, stated plainly: the `video` sources are still compiled and are no
longer checked.** They left `tools/gal/designs.ts`, `jedec/cupl.check.ts`'s registry,
`pins.check.ts`'s parts and consumers, `reach.check.ts`'s cards, ten scripts in
`npm run check`'s chain and five testbenches in `tools/sim/run.sh`'s default `TBS`.
...
a display list this machine does not have.** When `software/archive/demo/` is redesigned for
`video3`, the sources and the drawing follow their documents here.

⭐ **2026-09-23: they have moved now**, into this archive with the rest of the
card — term lists, `.pld`, fits and CUPL references in
[`video/logic/`](video/logic/), the generated and hand-written Verilog in
[`video/sim/`](video/sim/), the drawing in [`video/board/`](video/board/). Nothing
above changed except where the files are: `gen.ts` still emits them, `fold.check.ts`
still uses the arbiter as its fixture, and nothing checks the card. The machine's
[`docs/history.md`](../../docs/history.md) has the reorganisation.
...
`hardware/archive/video/docs/graphics.md` still says "the card is 33 ICs" in the present tense —
it was, on the day it stopped being a card. `hardware/tools/lib/docs.check.ts` exempts
`hardware/archive/` for the same reason it exempts `history.md` and the design reviews.
