...
[`hardware/archive/video/logic/video.cpld.ts`](logic/video.cpld.ts) and
`hardware/<card>/logic/cpld/`, with every design also checked against Atmel's own compiler by
...
`hardware/archive/video/board/video.circuit.tsx` since 2026-09-08): a 1 kΩ/2 kΩ ladder, three NPN
...
`vsup` is 63 of 64 I/O.
Their cell counts are **93**, 113 and 94 of 128, and `vctrl` places on the fitter's
...
count and a pin count; only `hardware/archive/video/logic/cpld/vaddr.fit` shows it.
...
⚠ **`vsup`'s spare room is 34 macrocells and 1 pin** (`cpld/vsup.fit`: 94 of 128
cells, 63 of 64 I/O), which is the shape of every
