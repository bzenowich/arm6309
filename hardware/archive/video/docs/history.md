...
`software/nitros9/docs/nitros9-hardware-improvements.md` items H7, H8 and H14 were built: `vsup`'s `LGO`
...
`software/archive/demo/bench/calib.asm` listed `HSCROLL` = 1, 2, 3, 5, 6 and 7 on the whole machine
...
All four are declared active-low now. `hardware/tools/gal/pins.check.ts` holds each pin to its
...
> **56 of 64 I/O, 127 of 128 cells** (`hardware/archive/video/logic/cpld/vctrl.fit`)
...
> (`JTAG=on hardware/tools/gal/prjbureau/fit1508.sh`), and no logic signal is placed on them.
...
> [`hardware/archive/video/logic/access.jedec.ts`](../logic/access.jedec.ts), because a
...
> `archive/video/logic/access.check.ts` asserts the float on a read.
...
> **`vctrl`** — sync (§6.2.1's polarity, VBL IRQ), sequencer, span control, the spare-access arbiter (§10.1.6.3), `CTRL`, span-mask handling, §19 item 35's blanking delay. **61 of 64 I/O, 104 of 128 cells** (`hardware/archive/video/logic/cpld/vctrl.fit`) | |

**§14 — the vsup row said:**

> so a third PLCC-84 is −2 packages before it does anything else. **54 of 64 I/O, 84 of 128 cells** (`hardware/archive/video/logic/cpld/vsup.fit`) | |
...
>     63 of 64 to 59 of 64** (`archive/video/logic/cpld/vaddr.fit`), which is more than the cell it
...
>     can close that but the netlist**, and `archive/video/board/video.circuit.tsx` does not carry
>     those nets yet.
...
(`hardware/tools/sim/machine_tb.sv`, `software/boot/boot.asm`). Twelve testbenches
...
re-fit with *"Design fits successfully"*; `archive/video/logic/cpld/vctrl.fit` and `vsup.fit` are the
authority.
...
**And the board length went back.** §14.2 recorded *"`hardware/tools/place` puts it on an
...
point.** `seqctl` is a *superseded* GAL design kept as derivation (`tools/gal/README.md`); the
...
figure is 99.1 cm² on an 18 cm board, measured by `hardware/tools/place`.
...
decode GALs — `hardware/tools/gal/README.md` put it at roughly twenty). The GAL-build
...
SMD (no socket), and `hardware/tools/gal/jedec/` stops applying — its assembler and
...
`rfa` was built (`archive/video/logic/regfile.jedec.ts`): moving `RA0`–`RA4` + `WSTB` off `vctrl`
...
each of audio's IC counts in turn (36 at the split; see `hardware/audio/docs/history.md`).
...
(§10.1.6's absorption list and `hardware/tools/place/parts.ts` are the record — the
...
now reconciled at **27** (verified against `hardware/tools/place/parts.ts`).
...
**Was:** *"the motherboard's own full census … is `hardware/mainboard/docs/ram.md` §6.5, at **17
ICs**"*.
...
8. **Logic fit — closed.** The sync section was fitted 2026-09-06
   ([`hardware/archive/video/logic/sync.jedec.ts`](../logic/sync.jedec.ts), fuse-level
   checked over whole frames in both families by `npm run check:sync`) and the
   scan-address pair the same day
   ([`hardware/archive/video/logic/scan.jedec.ts`](../logic/scan.jedec.ts), 17 of 20 with
...
12. **Span-wrap behaviour at the 1024-byte row boundary — decided 2026-09-06: wrap
    ...
    **The fit.** `WPTR` is nineteen bits with the same `{row, column}` structure as
    ...
    ([`hardware/archive/video/logic/access.jedec.ts`](../logic/access.jedec.ts) `wcol`) is
...
15. **Tile-mode fit — closed by the CPLD build.** §6.4.2's Variant A is v1 hardware
    ...
    [`video.parts.ts`](../logic/video.parts.ts), the
    ...
    **(c) The fetch cadence — closed 2026-09-08.** It was a placeholder: `TC0..TC2`
    ...
    [`cadence.check.ts`](../logic/cadence.check.ts) runs a whole line
...
16. **The tile fetch's fine-scroll behaviour — implemented, and it is
    ...
    `MAPSEL` in [`video.parts.ts`](../logic/video.parts.ts).
...
20. **§5.2.1's arbiter — closed 2026-09-06.** Fitted in
    [`hardware/archive/video/logic/access.jedec.ts`](../logic/access.jedec.ts) and checked
...
23. **The phase-dependent `FCLK` equations, and the sequencer's other half —
    ...
    **(b) The register-file decode — 21 macrocells, 28 product terms.** Item 23's own
    ...
    [`hardware/archive/video/logic/regfile.ts`](../logic/regfile.ts):
    ...
    *address* onto its own `GAL22V10` (`rfa` — `hardware/archive/video/logic/rfa.pld`) bought
    ...
    `hardware/<card>/logic/cpld/*.fit` files).
...
a **6809**; `hardware/cpu/src/` is the timing spike and holds no instruction table; and A09 was
...
`software/archive/demo/` scrolled a tilemap by half a cell on the whole machine and found item 48:
...
48. **⛔ OPEN 2026-09-13 — cell mode draws the first half of every cell from the previous
    cell's code when `HSCROLL[2:0]` = 4.** Found by `software/archive/demo/` on the whole machine
    ...
    of the cell to their left, and columns 4–7 are right. The check is `software/archive/demo/tools/checkdemo.py`,
...
**58 of 64 I/O, 113 of 128 cells** (`hardware/archive/video/logic/cpld/vaddr.fit`)

### video/docs/graphics.md — §14, the vctrl row

**52 of 64 I/O, 123 of 128 cells** (`hardware/archive/video/logic/cpld/vctrl.fit`) — ⚠ the fitter's first pass no longer places it and the second, with cascade logic, reports the cells; §19 item 46.
