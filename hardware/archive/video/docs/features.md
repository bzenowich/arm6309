...
address bits, so there is no adder anywhere. `hardware/archive/video/logic/tile.check.ts` asserts that
...
`hardware/archive/video/logic/tile.check.ts` walks every displayed pixel at whole-cell offsets,
...
`hardware/archive/video/logic/cadence.check.ts` runs a whole line and a whole frame in each of the
four modes.
...
> pin, and the part had exactly enough left.** `hardware/archive/video/logic/seqctl.jedec.ts`,
> `npm run check:seqctl`, and `archive/video/logic/cpld/vctrl.fit`.
...
| [`../../../hardware/gal/`](../logic/) | `tile.check.ts` (the no-adder property), `seqctl.check.ts` (the 13 writes per cell), `vctrl.pld` (the fitted sequencer and arbiter) |
