...
// AND the audio card, running software/archive/demo/ out of the boot ROM, with the
...
//                for hardware/audio/tools/dacwav
//   card.trace   every audio-card register write, numbered by tick exactly as
//                hardware/audio/refplayer numbers its own - so the whole run's register
...
module demo_tb;
  ...
  // the show's checkpoint and raster phase (software/archive/demo/gui.asm, $C600 and
