...
# machine on a quiet host - the show is 110 s. software/emu/run-emu.sh runs
...
echo
echo "-- the register stream at the backplane, against hardware/audio/refplayer"
...
# ⚠ AND NOT PAST THE AUDIO PLAYER'S STOP. software/archive/demo/gui.asm's STOP writes
...
cc -O2 -w -I"$ROOT/hardware/audio/refplayer" -o "$OUT/tracewav" "$ROOT/software/archive/demo/tools/tracewav.c" \
   "$ROOT/hardware/audio/refplayer/card.c" "$ROOT/hardware/audio/refplayer/mod_load.c" \
   "$ROOT/hardware/audio/refplayer/render.c" -lm
...
python3 "$ROOT/hardware/audio/tools/modcompare/abcompare.py" "$ROOT/software/demo/build/demo.mod" --seconds "$LEN" \
...
python3 "$ROOT/software/archive/demo/tools/checkdemo.py" "$OUT" "$ROOT/software/demo/build" || fail=1

echo
echo "-- the file"
python3 "$ROOT/software/archive/demo/tools/mkvideo.py" "$OUT" "$OUT/demo.mp4" || fail=1
exit $fail
