# `hardware/archive/video/bench/` — the retired card's benches, frozen
...
2026-09-20 — plus the ones for `software/archive/demo/`, the bare-metal show that only ever
...
| | |
|---|---|
| `run-vid.sh` | the NitrOS-9 **video console** on the host emulator, against `vtmodel.py`/`vgmodel.py` pixel for pixel, plus the IRQ-masked time. `software/nitros9/docs/nitros9-av-plan.md` P1 and P2 |
| `run-video.sh` | the progress video — one scripted session, screen and serial console side by side, `nitros9-progress.mp4` |
| `run-demo.sh` | ⭐ **the whole machine with both cards**, `demo_tb`, ~4.5 h: `software/archive/demo`'s show out of ROM, the audio A/B'd against libopenmpt, every checkpoint frame against `show.Model` |
...
**The card's design sources are still live in `hardware/archive/video/logic/`**, and
...
gone** — so `hardware/archive/video/sim/`'s `vaddr.v`, `vctrl.v`, `vsup.v`, `rfa.v`, `vlen.v` and
`pxsel.v` are now compiled by `gen.ts` and executed by **nothing**.

⚠ `software/archive/demo/` itself stays where it is, and not out of sentiment:
...
`software/emu/machine.c` is likewise the host emulator every live bench still
