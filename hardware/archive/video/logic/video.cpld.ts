...
import { merge, rename, toCupl, withActiveLow, type Merged } from "../../../tools/gal/jedec/cupl"
 ...
 * (mainboard/logic/clkdec.pld), so a span re-arms itself on every dot of its own wait:
...
const ctrlFanout: Cell[] = [
   ...
   * FRAME ENDED, since 2026-09-14 (software/nitros9/docs/nitros9-hardware-improvements.md H8).
   *
...
export const vctrlCpld: Merged = withActiveLow(merge(
   ...
   ...maskSerialiser, ...tileCadence, ...decodeCells],
  {
     ...
     *
     * `JTAG=on tools/gal/prjbureau/fit1508.sh archive/video/logic/vctrl.pld` is what says so - the
