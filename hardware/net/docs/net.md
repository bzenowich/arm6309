...
| [`hardware/storage/docs/sdcard.md`](../../storage/docs/sdcard.md) | §4.2's argument that a `TFM` against RAM is idempotent is what §3.2 rests on, and §11.1 is the same decision this card took. **Both cards keep their buffers in `A20 = 1`** |
| [`hardware/README.md`](../../README.md) | the 72-pin slot, and the six-slot count that decides §12 |
...
`hardware/tools/gal/README.md` gives the house cycle: within one bus cycle the order is
...
[`hardware/mainboard/logic/mmu.pld`](../../mainboard/logic/mmu.pld) already uses for the motherboard's
...
**12 ICs.** For scale ([`hardware/tools/place/parts.ts`](../../tools/place/parts.ts)):
...
1. **The slot budget forbids the 74xx design.** `applenet`'s v2 measured a discrete
   ...
   [`hardware/README.md`](../../README.md) gives the machine **six slots, five
...
[`hardware/io/README.md`](../../io/README.md) derives the working version —
...
**It cannot be built on this backplane.** [`hardware/tools/lib/slot.ts`](../../tools/lib/slot.ts)
...
`hardware/io/README.md` sets the test: *"what decides each case is whether a period part exists
...
| [`hardware/storage/docs/sdcard.md`](../../storage/docs/sdcard.md) | §3.4 phantom reads, §4 the `TFM` hazard, §6.2 the prefetched port idiom, §11.6 the CPU-side fix |
| [`hardware/io/README.md`](../../io/README.md) | the "does a period part fit the I/O budget?" test that §13.6 applies and fails |
| [`hardware/audio/docs/audio.md`](../../audio/docs/audio.md) | §8.1 the `ATF1508AS` open-collector `/IRQ` output, §9.3 the `'574` that drives the bus itself |
| [`hardware/README.md`](../../README.md) | the 72-pin slot, the six-slot count §12 depends on, and the `$FF` map as data |
| [`hardware/tools/gal/README.md`](../../tools/gal/README.md) | the house bus cycle — E-fall, Q-rise, E-rise, Q-fall — that §4.2 and §4.3 are written against |
