...
> `video3/bench/mkstardew.py`, and its bench is `video3/bench/run-v3star.sh` with
> `checkv3star.py`. ⭐ The desktop's Applications menu launches it
> ([`../../docs/boot-and-desktop.md`](../../docs/boot-and-desktop.md) §3.2).
> Superseded numbers are in [`history.md`](history.md).
...
`video3/bench/run-v3star.sh`: four runs, three mutations and two negative controls.
...
1. **The art** — done. `video3/bench/mkstardew.py`: a 1024 × 480 farm, 200 colours by
...
2. ⚠ **Whether the dither survives the tint — measured, and it does.**
   `python3 video3/bench/mkstardew.py --tint-test` renders one region (the pond's depth
