...
#
#   sh video3/bench/run-v3pin.sh          ~25 min.  OUT=dir, FRAMES=n, MODES=...
#
...
#
# ⭐ AND SINCE 2026-09-21 THE TABLE IS A FILE.  video3/bench/pcbtable.pic is
...
# (software/nitros9/mksddisk.sh, video3/bench/run-v3sd.sh), and the 488 K ROM
...
PD="$OUT/pindata"
rm -rf "$PD"; mkdir -p "$PD"
cp video3/bench/pcbtable.pic video3/bench/pcbtable.pal "$PD/"
...
claim "the picture on the card is byte for byte the one the gate compares against" \
  sh -c "os9 copy -o=0 '$OUT/sd.img,DATA/pcbtable.pic' '$OUT/onthecard.pic' && cmp -s video3/bench/pcbtable.pic '$OUT/onthecard.pic'"
claim "⭐ and mkrom.sh puts it in the demo DATA set, so the DESKTOP can fork this scene" \
  sh -c "cmp -s video3/bench/pcbtable.pic '$OUT/data/pcbtable.pic' && cmp -s video3/bench/pcbtable.pal '$OUT/data/pcbtable.pal'"
