...
"""mkpcb.py - the pinball table as a 640 x 512 PICTURE, themed as a 1984 PCB.
    ...
    python3 video3/bench/mkpcb.py          # ~20 s, writes into video3/bench/
...
nothing on its own.
    ...
    python3 video3/bench/checkpcb.py --negative     # ~20 s; the exit code is
                                                    # the answer
...
has a working SD card (`storage/docs/sdcard.md`, driver `rbsd`), so the
