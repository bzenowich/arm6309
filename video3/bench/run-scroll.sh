...
#
#   sh video3/bench/run-scroll.sh       ~12 min.  OUT=dir, FRAMES=n
#
# `scroll` (video3/docs/scrolling.md, and the port's level2/arm6309/cmds/
# scroll.asm) is the other answer to the problem `zelda` solves by being a
...
# the rotation has left them.  `checkscroll.py` walks the same path the
...
if [ -z "$NOBUILD" ]; then
  python3 video3/bench/mkscroll.py "$NITROS9DIR/level2/arm6309/cmds" \
    > "$OUT/mkart.log" 2>&1 || {
...
SD="$OUT/scrolldata"
rm -rf "$SD"; mkdir -p "$SD"
cp video3/bench/scroll.bnk "$SD/"
DATA="$SD" sh software/nitros9/mksyscard.sh "$OUT/sd.img" scroll \
...
run() {
  ...
  printf 'chx /sd0/cmds\riniz w5\rscroll %d %d >/w5\recho DONE-arm6309\r' \
...
for r in $RUNS; do
  case "$r" in
    m0) echo "--- m0: the engine, and it must be exact ---"
        python3 video3/bench/checkscroll.py "$OUT/m0" "$FRAMES" || fail=1 ;;
    m1) echo "--- m1 ⛔ MUTATION: the strip is not moved - this must FAIL ---"
        if python3 video3/bench/checkscroll.py "$OUT/m1" "$FRAMES" > "$OUT/m1.txt" 2>&1
    ...
    m2) echo "--- m2 ⛔ MUTATION: no vertical band - this must FAIL ---"
        if python3 video3/bench/checkscroll.py "$OUT/m2" "$FRAMES" > "$OUT/m2.txt" 2>&1
...
echo
[ "$fail" = 0 ] && echo "ok    run-scroll.sh: the engine is byte-exact and both mutations were caught"
[ "$fail" = 0 ] || echo "FAIL  run-scroll.sh"
exit "$fail"
