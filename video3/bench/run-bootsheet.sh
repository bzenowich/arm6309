...
# CONTACT SHEET.
#
#   sh video3/bench/run-bootsheet.sh          (from the repository root)
...
# LIKE doing the thing the owner asked for (docs/boot-and-desktop.md): the ROM
...
( cd "$OUT" && SERIAL_IN=typed.txt SERIAL_GATE="02}" SERIAL_TYPE=60 SERIAL_THINK=700 \
    ...
    PS2_SCRIPT="$ROOT/video3/bench/scripts/bootsheet.ps2" PS2_SCRIPT_GATE="DESK-READY" \
...
# software/demo/emu is a LOGIC model that boots NitrOS-9; it does not run the
...
DLG=${DLG:-$ROOT/hardware/gal/verilog}
python3 video3/bench/bootsheet.py "$OUT" \
