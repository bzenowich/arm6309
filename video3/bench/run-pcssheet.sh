...
#
#   sh video3/bench/run-pcssheet.sh          OUT=dir  BATCH=6
#
...
N=$(python3 -c "import sys;sys.path.insert(0,'video3/bench');import pcsfile;print(len(pcsfile.demo_tables()))")
...
while [ "$i" -lt "$N" ]; do
  ...
  echo "--- tables $i..$j ---"
  PCSTBLS="$i-$j" python3 video3/bench/mkpcs.py "$NITROS9DIR/level2/arm6309/cmds" \
...
echo
python3 video3/bench/pcssheet.py "$OUT" "$OUT/tables.png"
