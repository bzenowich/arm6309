...
#
#   sh video3/bench/run-v3copy.sh [seconds]        OUT=dir overrides
#
...
OUT=${OUT:-/tmp/arm6309-v3copy}
B=software/demo/build
mkdir -p "$B" "$OUT"
sh software/tools/fetch-a09.sh > /dev/null
A09="${A09_DIR:-/tmp/arm6309-a09}/a09"
python3 video3/bench/mkv3copy.py video3/bench/v3copy.asm video3/bench/v3copy.json > /dev/null
"$A09" -B$B/v3copy.bin -L$B/v3copy.lst video3/bench/v3copy.asm > $B/v3copy.log 2>&1 || { cat $B/v3copy.log; exit 1; }
...
python3 video3/tools/v3model.py "$OUT/frames.bin" video3/bench/v3copy.json
