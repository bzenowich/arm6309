...
"""bootsheet.py OUT - a contact sheet of the machine BOOTING, power-on to desktop.
    ...
    python3 video3/bench/bootsheet.py /tmp/arm6309-bootsheet
...
def dialog_tiles(paths, font, small):
    ...
    ⛔ software/demo/emu does not run the POST's video sections at all - its
...
def emu_label(m, names):
    ...
    The POST's $FF2F writes are a machine_tb fact: software/demo/emu boots
    ...
    """
    if m["prog"]:
        return "software/demo/emu - " + label_for(m["prog"], names)
    return "software/demo/emu - NitrOS-9 and the desktop (this model runs no POST)"
...
if __name__ == "__main__":
    ...
    if "--dialog" in a:
        ...
        dl = a[i + 1:]
        a = a[:i]
    sys.exit(sheet(a[0] if a else "/tmp/arm6309-bootsheet", dl))
