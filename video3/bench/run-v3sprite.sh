...
#
#   sh video3/bench/run-v3sprite.sh [seconds]        OUT=dir overrides
#
...
ROOT=$(pwd)
SECS=${1:-10}
OUT=${OUT:-/tmp/arm6309-v3sprite}
B=software/demo/build
mkdir -p "$B" "$OUT"
sh software/tools/fetch-a09.sh > /dev/null
A09="${A09_DIR:-/tmp/arm6309-a09}/a09"
python3 video3/bench/mkv3sprite.py video3/bench/v3sprite.asm video3/bench/v3sprite.json > /dev/null
"$A09" -B$B/v3sprite.bin -L$B/v3sprite.lst video3/bench/v3sprite.asm > $B/v3sprite.log 2>&1 || { cat $B/v3sprite.log; exit 1; }
...
python3 video3/tools/v3model.py "$OUT/frames.bin" video3/bench/v3sprite.json
