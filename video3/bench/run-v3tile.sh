...
#
#   sh video3/bench/run-v3tile.sh [seconds]        OUT=dir overrides
#
...
ROOT=$(pwd)
SECS=${1:-16}
OUT=${OUT:-/tmp/arm6309-v3tile}
B=software/demo/build
mkdir -p "$B" "$OUT"
sh software/tools/fetch-a09.sh > /dev/null
A09="${A09_DIR:-/tmp/arm6309-a09}/a09"
python3 video3/bench/mkv3tile.py video3/bench/v3tile.asm video3/bench/v3tile.json > /dev/null
"$A09" -B$B/v3tile.bin -L$B/v3tile.lst video3/bench/v3tile.asm > $B/v3tile.log 2>&1 || { cat $B/v3tile.log; exit 1; }
...
python3 video3/tools/v3model.py "$OUT/frames.bin" video3/bench/v3tile.json
